...
class BookmarkManager(QtWidgets.QWidget):
    ...
    def __init__(self, app, storage, parent=None):
        ...
        self.table_widget.setHorizontalHeaderLabels(["#", _("Title"), _("Web Link")])
        ...
        for bookmark in self.bm_dict.values():
            if title == bookmark[0] or link == bookmark[1]:
                self.app.inform.emit(
                    "[ERROR_NOTCL] %s" % _("Either the Title or the Weblink already in the table.")
                )
                return "fail"
    ...
    def on_export_bookmarks(self):
        ...
        filename, _f = FCFileSaveDialog.get_saved_filename(
            ...
            directory="{l_save}/{n}_{date}".format(
                l_save=str(self.app.get_last_save_folder()), n=_("Bookmarks"), date=date
            ),
            ext_filter=filter__,
        )
    ...
    def on_import_bookmarks(self):
        ...
        filename, _f = QtWidgets.QFileDialog.getOpenFileName(
            caption=_("Import Bookmarks"), filter=filter_
        )
    ...
    def rebuild_actions(self):
        ...
        for row in range(self.table_widget.rowCount()):
            ...
            entry = int(row) + 1
            self.bm_dict.update({str(entry): [title, wlink]})
