...
class ScriptObject(FlatCAMObj):
    ...
    def set_ui(self, ui):
        ...
        assert isinstance(self.ui, ScriptObjectUI), "Expected a ScriptObjectUI, got %s" % type(
            self.ui
        )
        ...
        self.script_editor_tab.buttonOpen.clicked.connect(
            lambda: self.script_editor_tab.handleOpen(filt=flt)
        )
        self.script_editor_tab.buttonSave.clicked.disconnect()
        self.script_editor_tab.buttonSave.clicked.connect(
            lambda: self.script_editor_tab.handleSaveGCode(filt=flt)
        )
