...
class FlatCAMLineVisual(LineVisual):
    def __init__(
        ...
        LineVisual.__init__(
            self, pos=pos, color=color, width=width, connect=connect, method=method, antialias=True
        )
...
def _update_shape_buffers(data, triangulation="glu"):
    ...
    geo, color, face_color, tolerance = (
        data["geometry"],
        data["color"],
        data["face_color"],
        data["tolerance"],
        ...
        if type(geo) == LineString:
            ...
            if face_color is not None:
                if triangulation == "glu":
                    ...
                    print(
                        "Triangulation type '%s' isn't implemented. Drawing only edges."
                        % triangulation
                    )
...
class ShapeCollectionVisual(CompoundVisual):
    ...
    def __init__(self, linewidth=1, triangulation="vispy", layers=3, pool=None, **kwargs):
        ...
        visuals_ = [
            self._lines[i // 2] if i % 2 else self._meshes[i // 2] for i in range(0, layers * 2)
        ]
                    ...
                    if new_mesh_color and new_mesh_color != "":
                        ...
                        if dim_mesh_tris != 0:
                            try:
                                ...
                                data["mesh_colors"] = [
                                    mesh_color_rgba for __ in range(len(data["mesh_colors"]))
                                ]
                            except Exception as e:
                                print(
                                    "VisPyVisuals.ShapeCollectionVisual.update_color(). "
                    ...
                    if new_line_color and new_line_color != "":
                        ...
                        if dim_line_pts != 0:
                            try:
                                ...
                                data["line_colors"] = [
                                    mesh_color_rgba for __ in range(len(data["line_colors"]))
                                ]
                            except Exception as e:
                                print(
                                    "VisPyVisuals.ShapeCollectionVisual.update_color(). "
            ...
            for k, data in list(self.data.items()):
                if data["visible"] and "line_pts" in data:
                    ...
                    if k in indexes:
                        ...
                        if dim_mesh_tris != 0:
                            try:
                                mesh_colors[data["layer"]] += [
                                    Color(data["face_color"]).rgba
                                ] * dim_mesh_tris
                            except Exception as e:
                                print(
                                    "VisPyVisuals.ShapeCollectionVisual.update_color(). "
        ...
        for data in list(self.data.values()):
            if data["visible"] and "line_pts" in data:
                try:
                    ...
                    mesh_tris[data["layer"]] += [
                        x + len(mesh_vertices[data["layer"]]) for x in data["mesh_tris"]
                    ]
                    mesh_vertices[data["layer"]] += data["mesh_vertices"]
                    mesh_colors[data["layer"]] += data["mesh_colors"]
                except Exception as e:
                    print(
                        "VisPyVisuals.ShapeCollectionVisual._update() --> Data error. %s" % str(e)
                    )
    ...
    def redraw(self, indexes=None, update_colors=None):
        ...
        for i in list(self.data.keys()) if not indexes else indexes:
            if i in list(self.results.keys()):
                try:
                    ...
                    print(
                        "VisPyVisuals.ShapeCollectionVisual.redraw() --> Data error = %s. Indexes = %s"
                        % (str(e), str(indexes))
                    )
        ...
        if update_colors is None or update_colors is False:
            ...
            try:
                ...
                print(
                    "VisPyVisuals.ShapeCollectionVisual.redraw() --> Update colors error = %s."
                    % str(e)
                )
