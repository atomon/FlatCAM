...
class GerberGenPrefGroupUI(OptionsGroupUI):
    def __init__(self, decimals=4, parent=None):
        ...
        self.plot_options_label.setToolTip(_("Plot (show) this object."))
        ...
        self.gerber_units_label.setToolTip(_("The units used in the Gerber file."))

        self.gerber_units_radio = RadioSet(
            [{"label": _("Inch"), "value": "IN"}, {"label": _("mm"), "value": "MM"}]
        )
        self.gerber_units_radio.setToolTip(_("The units used in the Gerber file."))
        ...
        self.gerber_zeros_radio = RadioSet(
        ...
        self.clear_colors_button.setToolTip(_("Reset the colors associated with Gerber objects."))
        ...
        self.gerber_alpha_label.setToolTip(_("Set the fill transparency for plotted objects."))
    ...
    def on_fill_color_changed(self):
        self.app.defaults["gerber_plot_fill"] = (
            self.fill_color_entry.get_value()[:7] + self.app.defaults["gerber_plot_fill"][7:9]
        )

    def on_gerber_alpha_changed(self, spinner_value):
        self.app.defaults["gerber_plot_fill"] = self.app.defaults["gerber_plot_fill"][:7] + (
            hex(spinner_value)[2:] if int(hex(spinner_value)[2:], 16) > 0 else "00"
        )
        self.app.defaults["gerber_plot_line"] = self.app.defaults["gerber_plot_line"][:7] + (
            hex(spinner_value)[2:] if int(hex(spinner_value)[2:], 16) > 0 else "00"
        )

    def on_line_color_changed(self):
        self.app.defaults["gerber_plot_line"] = (
            self.line_color_entry.get_value()[:7] + self.app.defaults["gerber_plot_line"][7:9]
        )

    def on_colors_clear_clicked(self):
        self.app.defaults["gerber_color_list"].clear()
        self.app.inform.emit(
            "[WARNING_NOTCL] %s" % _("Stored colors for Gerber objects are deleted.")
        )
