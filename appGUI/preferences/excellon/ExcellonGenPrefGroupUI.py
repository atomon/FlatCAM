...
class ExcellonGenPrefGroupUI(OptionsGroupUI):
    ...
    def __init__(self, decimals=4, parent=None):
        ...
        self.solid_cb.setToolTip("Plot as solid circles.")
        ...
        self.excellon_zeros_radio = RadioSet(
            [{"label": _("LZ"), "value": "L"}, {"label": _("TZ"), "value": "T"}]
        )
        ...
        self.excellon_units_radio = RadioSet(
        ...
        self.line_color_label.setToolTip(_("Set the line color for plotted objects."))
        ...
        self.excellon_alpha_label.setToolTip(_("Set the fill transparency for plotted objects."))
        ...
        if current_platform == "64bit":
            self.excellon_optimization_radio.setOptionsDisabled(
                [_("MetaHeuristic"), _("Basic")], False
            )
            self.optimization_time_label.setDisabled(False)
            self.optimization_time_entry.setDisabled(False)
        else:
            self.excellon_optimization_radio.setOptionsDisabled(
                [_("MetaHeuristic"), _("Basic")], True
            )
    ...
    def on_fill_color_entry(self):
        self.app.defaults["excellon_plot_fill"] = (
            self.fill_color_entry.get_value()[:7] + self.app.defaults["excellon_plot_fill"][7:9]
        )

    def on_line_color_entry(self):
        self.app.defaults["excellon_plot_line"] = (
            self.line_color_entry.get_value()[:7] + self.app.defaults["excellon_plot_line"][7:9]
        )

    def on_excellon_alpha_changed(self, spinner_value):
        self.app.defaults["excellon_plot_fill"] = self.app.defaults["excellon_plot_fill"][:7] + (
            hex(spinner_value)[2:] if int(hex(spinner_value)[2:], 16) > 0 else "00"
        )
        self.app.defaults["excellon_plot_line"] = self.app.defaults["excellon_plot_line"][:7] + (
            hex(spinner_value)[2:] if int(hex(spinner_value)[2:], 16) > 0 else "00"
        )

    def on_excellon_defaults_button(self):
        self.app.preferencesUiManager.defaults_form_fields["excellon_format_lower_in"].set_value(
            "4"
        )
        self.app.preferencesUiManager.defaults_form_fields["excellon_format_upper_in"].set_value(
            "2"
        )
        self.app.preferencesUiManager.defaults_form_fields["excellon_format_lower_mm"].set_value(
            "3"
        )
        self.app.preferencesUiManager.defaults_form_fields["excellon_format_upper_mm"].set_value(
            "3"
        )
    ...
    def on_update_exc_export(self, state):
        ...
        if state:
            ...
            try:
                self.excellon_format_upper_in_entry.returnPressed.disconnect(
                    self.on_excellon_format_changed
                )
            except TypeError:
                pass
            try:
                self.excellon_format_lower_in_entry.returnPressed.disconnect(
                    self.on_excellon_format_changed
                )
            except TypeError:
                pass
            try:
                self.excellon_format_upper_mm_entry.returnPressed.disconnect(
                    self.on_excellon_format_changed
                )
            except TypeError:
                pass
            try:
                self.excellon_format_lower_mm_entry.returnPressed.disconnect(
                    self.on_excellon_format_changed
                )
            except TypeError:
                pass
            ...
            try:
                self.excellon_zeros_radio.activated_custom.disconnect(
                    self.on_excellon_zeros_changed
                )
            except TypeError:
                pass
            try:
                self.excellon_units_radio.activated_custom.disconnect(
                    self.on_excellon_zeros_changed
                )
            except TypeError:
                pass
            ...
            # the connect them
            self.excellon_format_upper_in_entry.returnPressed.connect(
                self.on_excellon_format_changed
            )
            self.excellon_format_lower_in_entry.returnPressed.connect(
                self.on_excellon_format_changed
            )
            self.excellon_format_upper_mm_entry.returnPressed.connect(
                self.on_excellon_format_changed
            )
            self.excellon_format_lower_mm_entry.returnPressed.connect(
                self.on_excellon_format_changed
            )
