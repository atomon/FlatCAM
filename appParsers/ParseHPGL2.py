...
class HPGL2:
    ...
    def __init__(self, app):
        ...
        self.arc_3pt_re = re.compile(
            r"^AT\s*([+-]?\d+),?\s*([+-]?\d+),?\s*([+-]?\d+),?\s*([+-]?\d+);?$"
        )
    ...
    def parse_lines(self, glines):
        ...
        self.app.inform.emit(
            "%s %d %s." % (_("HPGL2 processing. Parsing"), len(glines), _("Lines").lower())
        )
        try:
            for gline in glines:
                ...
                if self.init_done is True:
                    ...
                    if match:
                        ...
                        if current_x is not None and current_y is not None:
                            ...
                            geo = Point((current_x, current_y)).buffer(
                                radius, int(self.steps_per_circle)
                            )
            ...
            self.app.inform.emit(
                "%s: %d." % (_("Gerber processing. Joining polygons"), len(geo_buffer))
            )
