    ...
    **USAGE**::
    ...
    def aperture_parse(self, apertureId, apertureType, apParameters):
        ...
        * *Aperture Macro (AM)*: macro (ApertureMacro), modifiers (list)
        ...
        if apertureType == "C":  # Circle, example: %ADD11C,0.1*%
            self.apertures[apid] = {"type": "C", "size": float(paramList[0])}
            return apid
        ...
        if apertureType == "R":  # Rectangle, example: %ADD15R,0.05X0.12*%
            self.apertures[apid] = {
                ...
                "size": np.sqrt(float(paramList[0]) ** 2 + float(paramList[1]) ** 2),
            }  # Hack
            return apid
    ...
    def parse_lines(self, glines):
        ...
        self.app.inform.emit(
            "%s %d %s." % (_("Gerber processing. Parsing"), len(glines), _("Lines").lower())
        )
        try:
            for gline in glines:
                if self.app.abort_flag:
                ...
                if match:
                    ...
                    if path_length > 1 and current_polarity != new_polarity:
                        ...
                        geo_s = LineString(path).buffer(
                            width / 1.999, int(self.steps_per_circle / 4)
                        )
                    ...
                    if buff_length > 0:
                        if current_polarity == "D":
                            self.solid_geometry = self.solid_geometry.union(
                                unary_union(poly_buffer)
                            )

                        else:
                            self.solid_geometry = self.solid_geometry.difference(
                                unary_union(poly_buffer)
                            )
                ...
                if match:
                    ...
                    log.debug(
                        "Gerber format found. Coordinates type = %s (Absolute or Relative)"
                        % absolute
                    )
                    continue
                ...
                if match:
                    ...
                    log.warning(
                        "Gerber obsolete coordinates type found = %s (Absolute or Relative) "
                        % absolute
                    )
                    continue
                    ...
                    if current_operation_code == 3:
                        ...
                        try:
                            ...
                            if not flash.is_empty:
                                ...
                                self.apertures[current_aperture]["geometry"].append(
                                    deepcopy(geo_dict)
                                )

                        except IndexError:
                            log.warning(
                                "Line %d: %s -> Nothing there to flash!" % (line_num, gline)
                            )
                ...
                if match:
                    ...
                    if path_length > 1:
                        if self.apertures[last_path_aperture]["type"] == "R":
                            ...
                            if "geometry" not in self.apertures[last_path_aperture]:
                                self.apertures[last_path_aperture]["geometry"] = []
                            self.apertures[last_path_aperture]["geometry"].append(
                                deepcopy(geo_dict)
                            )
                ...
                if self.regionoff_re.search(gline):
                    ...
                    if current_aperture in self.apertures:
                        ...
                        region_geo = Polygon(
                            path
                        )  # Sprint Layout Gerbers with ground fill are crashed with above
                    else:
                ...
                if match:
                    ...
                    # Parse coordinates
                    if match.group(2) is not None:
                        linear_x = parse_gerber_number(
                            match.group(2), self.int_digits, self.frac_digits, self.gerber_zeros
                        )
                        current_x = linear_x
                    else:
                        linear_x = current_x
                    if match.group(3) is not None:
                        linear_y = parse_gerber_number(
                            match.group(3), self.int_digits, self.frac_digits, self.gerber_zeros
                        )
                        current_y = linear_y
                    else:
                        linear_y = current_y
                            ...
                            if making_region is False:
                                ...
                                try:
                                    if self.apertures[current_aperture]["type"] == "R":
                                        ...
                                        log.debug(
                                            "Coords: %s - %s - %s - %s" % (minx, miny, maxx, maxy)
                                        )
                            ...
                            if making_region:
                                ...
                                try:
                                    ...
                                    self.app.inform.emit(
                                        ...
                                        % (
                                            _(
                                                "Region does not have enough points. "
                                ...
                                if last_path_aperture is None:
                                    log.warning(
                                        "No aperture defined for curent path. (%d)" % line_num
                                    )
                                width = self.apertures[last_path_aperture][
                                    "size"
                                ]  # TODO: WARNING this should fail!
                                geo_s = LineString(path).buffer(
                                    width / 1.999, int(self.steps_per_circle / 4)
                                )
                        ...
                        if path_length > 1:
                            ...
                            if not geo_f.is_empty:
                                try:
                                    ...
                                    log.debug(
                                        "camlib.Gerber.parse_lines() --> G01 match D03 --> %s"
                                        % str(e)
                                    )
                                    follow_buffer.append(geo_f)
                                    geo_dict["follow"] = geo_f
                ...
                if match:
                    ...
                    try:
                        circular_x = parse_gerber_number(
                            circular_x, self.int_digits, self.frac_digits, self.gerber_zeros
                        )
                    except Exception:
                        circular_x = current_x

                    try:
                        circular_y = parse_gerber_number(
                            circular_y, self.int_digits, self.frac_digits, self.gerber_zeros
                        )
                    except Exception:
                    ...
                    try:
                        i = parse_gerber_number(
                            i, self.int_digits, self.frac_digits, self.gerber_zeros
                        )
                    except Exception:
                        i = 0

                    try:
                        j = parse_gerber_number(
                            j, self.int_digits, self.frac_digits, self.gerber_zeros
                        )
                    except Exception:
                        j = 0
                    ...
                    if quadrant_mode is None:
                        log.error(
                            "Found arc without preceding quadrant specification G74 or G75. (%d)"
                            % line_num
                        )
                        log.error(gline)
                        continue
                    ...
                    if mode is None and current_interpolation_mode not in [2, 3]:
                        log.error(
                            "Found arc without circular interpolation mode defined. (%d)" % line_num
                        )
                        log.error(gline)
                        continue
                    ...
                    if current_operation_code == 2:
                        ...
                        if path_length > 1:
                            ...
                            # this treats the case when we are storing geometry as solids
                            buffered = LineString(path).buffer(
                                width / 1.999, int(self.steps_per_circle)
                            )
                    ...
                    if quadrant_mode == "MULTI":
                        ...
                        if current_x == circular_x and current_y == circular_y:
                            ...
                            stop = np.arctan2(
                                -center[1] + circular_y, -center[0] + circular_x
                            )  # Stop angle
                    ...
                    if quadrant_mode == "SINGLE":
                        ...
                        for center in center_candidates:
                            ...
                            radius2 = np.sqrt(
                                (center[0] - circular_x) ** 2 + (center[1] - circular_y) ** 2
                            )
                            ...
                            log.debug(
                                "ARC START: %f, %f  CENTER: %f, %f  STOP: %f, %f"
                                % (
                                    current_x,
                                    current_y,
                                    center[0],
                            ...
                            log.debug(
                                "START Ang: %f, STOP Ang: %f, DIR: %s, ABS: %.12f <= %.12f: %s"
                                % (
                                    start * 180 / np.pi,
                                    stop * 180 / np.pi,
                                    arcdir[current_interpolation_mode],
            ...
            try:
                ...
                log.error(
                    "Object is not Gerber file or empty. Aborting Object creation. %s" % str(e)
                )
                return "fail"

            log.warning("Joining %d polygons." % buff_length)
            self.app.inform.emit(
                "%s: %d." % (_("Gerber processing. Joining polygons"), buff_length)
            )
        ...
        * ``regions``
        ...
        try:
            for apid in self.apertures:
                ...
                try:
                    if (
                        str(self.apertures[apid]["type"]) == "R"
                        or str(self.apertures[apid]["type"]) == "O"
                    ):
        ...
        if factor is None:
            ...
            def buffer_geom(obj):
                if type(obj) is list:
                    ...
                    try:
                        ...
                        return obj.buffer(
                            distance, resolution=int(self.steps_per_circle), join_style=join
                        )
            ...
            try:
                for apid in self.apertures:
                    ...
                    try:
                        if self.apertures[apid]["size"] is not None:
                            self.apertures[apid]["size"] = float(
                                self.apertures[apid]["size"] + (distance * 2)
                            )
                    except KeyError:
                        pass
            except Exception as e:
            ...
            try:
                for apid in self.apertures:
                    ...
                    try:
                        if self.apertures[apid]["size"] is not None:
                            self.apertures[apid]["size"] = (
                                float(self.apertures[apid]["size"]) * distance
                            )
                    except KeyError:
                        pass
                                ...
                                if isinstance(new_geo_el["follow"], Point):
                                    if str(self.apertures[apid]["type"]) == "C":
                                        new_geo_el["solid"] = geo_el["follow"].buffer(
                                            size / 1.9999, resolution=int(self.steps_per_circle)
                                        )
                                        ...
                                        log.debug(
                                            "appParsers.ParseGerber.Gerber.buffer() --> "
            ...
            for apid in self.apertures:
                if "geometry" in self.apertures[apid]:
                    new_solid_geo += [
                        geo_el["solid"] for geo_el in self.apertures[apid]["geometry"]
                    ]
...
def parse_gerber_number(strnumber, int_digits, frac_digits, zeros):
    ...
    if zeros == "T":
        ...
        ret_val = (int_val * (10 ** ((int_digits + frac_digits) - len(strnumber)))) * (
            10 ** (-frac_digits)
        )
