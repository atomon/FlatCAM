...
class PdfParser(QtCore.QObject):
    ...
    def __init__(self, app):
        ...
        # detect 'c' command
        self.draw_arc_3pt_re = re.compile(
            r"^(-?\d+\.?\d*)\s(-?\d+\.?\d*)\s(-?\d+\.?\d*)\s(-?\d+\.?\d*)\s(-?\d+\.?\d*)"
            r"\s(-?\d+\.?\d*)\s*c$"
        )
        # detect 'v' command
        self.draw_arc_2pt_c1start_re = re.compile(
            r"^(-?\d+\.?\d*)\s(-?\d+\.?\d*)\s(-?\d+\.?\d*)\s(-?\d+\.?\d*)\s*v$"
        )
        # detect 'y' command
        self.draw_arc_2pt_c2stop_re = re.compile(
            r"^(-?\d+\.?\d*)\s(-?\d+\.?\d*)\s(-?\d+\.?\d*)\s(-?\d+\.?\d*)\s*y$"
        )
        # detect 'h' command
        ...
        self.combined_transform_re = re.compile(
            r"^(q)?\s*(-?\d+\.?\d*) (-?\d+\.?\d*) (-?\d+\.?\d*) (-?\d+\.?\d*) "
            r"(-?\d+\.?\d*) (-?\d+\.?\d*)\s+cm$"
        )
    ...
    def parse_pdf(self, pdf_content):
        ...
        clear_apertures_dict = {"0": {"size": 0.0, "type": "C", "geometry": []}}
        ...
        for pline in lines:
            ...
            if match:
                ...
                if (
                    color[0] == old_color[0]
                    and color[1] == old_color[1]
                    and color[2] == old_color[2]
                ):
            ...
            if match:
                ...
                if (float(match.group(3)) == 0 and float(match.group(4)) == 0) and (
                    ...
                    log.debug(
                        "parse_pdf() --> OFFSET transformation found on line: %s --> %s"
                        % (line_nr, pline)
                    )
                ...
                if float(match.group(2)) != 1 and float(match.group(5)) != 1:
                    log.debug(
                        "parse_pdf() --> SCALE transformation found on line: %s --> %s"
                        % (line_nr, pline)
                    )
            ...
            if match:
                ...
                pt = (
                    x * self.point_to_unit_factor * scale_geo[0],
            ...
            if match:
                ...
                x = (
                    (float(match.group(1)) + offset_geo[0])
                    * self.point_to_unit_factor
                    * scale_geo[0]
                )
                y = (
                    (float(match.group(2)) + offset_geo[1])
                    * self.point_to_unit_factor
                    * scale_geo[1]
                )
                width = (
                    (float(match.group(3)) + offset_geo[0])
                    * self.point_to_unit_factor
                    * scale_geo[0]
                )
                height = (
                    (float(match.group(4)) + offset_geo[1])
                    * self.point_to_unit_factor
                    * scale_geo[1]
                )
                pt1 = (x, y)
                        ...
                        for subp in path["lines"]:
                            ...
                            try:
                                geo = LineString(geo).buffer(
                                    (float(applied_size) / 2), resolution=self.step_per_circles
                                )
                                path_geo.append(geo)
                            except ValueError:
                                pass
                ...
                if current_subpath == "bezier":
                    if path["bezier"]:
                        for subp in path["bezier"]:
                            geo = []
                            for b in subp:
                                geo += self.bezier_to_points(
                                    start=b[0], c1=b[1], c2=b[2], stop=b[3]
                                )
                            try:
                                geo = LineString(geo).buffer(
                                    (float(applied_size) / 2), resolution=self.step_per_circles
                    ...
                    if found_aperture:
                        for pdf_geo in path_geo:
                            if isinstance(pdf_geo, MultiPolygon):
                                for poly in pdf_geo:
                                    ...
                                    apertures_dict[copy(found_aperture)]["geometry"].append(
                                        deepcopy(new_el)
                                    )
                            else:
                        ...
                        for pdf_geo in path_geo:
                            if isinstance(pdf_geo, MultiPolygon):
                                for poly in pdf_geo:
                                    new_el = {"solid": poly, "follow": poly.exterior}
                                    apertures_dict[str(aperture)]["geometry"].append(
                                        deepcopy(new_el)
                                    )
                            else:
                                new_el = {"solid": pdf_geo, "follow": pdf_geo.exterior}
                        ...
                        for subp in path["lines"]:
                            ...
                            try:
                                geo_el = Polygon(geo).buffer(
                                    0.0000001, resolution=self.step_per_circles
                                )
