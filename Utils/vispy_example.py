...
class VisPyCanvas(scene.SceneCanvas):
    ...
    def __init__(self, config=None):
        ...
        # View & Camera
        self.view = self.grid_widget.add_view(
            row=1, col=1, border_color=tick_color, bgcolor=theme_color
        )
        ...
        self.rect = Rectangle(
            center=(65, 30),
            color=Color("#0000FF10"),
            border_color=Color("#0000FF10"),
            width=120,
            height=50,
            radius=[5, 5, 5, 5],
            parent=self.view,
        ...
        self.text = Text(
            "", parent=self.view, color="black", pos=(5, 30), method="gpu", anchor_x="left"
        )
...
class PlotCanvas(QtCore.QObject):
    ...
    def __init__(self, container, my_app):
        ...
        self.v_line = scene.visuals.InfiniteLine(
            pos=0, color=(0.0, 0.0, 1.0, 0.3), vertical=True, parent=self.vispy_canvas.view.scene
        )

        self.h_line = scene.visuals.InfiniteLine(
            pos=0, color=(0.00, 0.0, 1.0, 0.3), vertical=False, parent=self.vispy_canvas.view.scene
        )
...
class MyApp(QtCore.QObject):
    ...
    def on_mouse_move(self, event):
        ...
        self.ui.position_label.setText(
            "Position:  X: %.4f\tY: %.4f" % (pos_canvas[0], pos_canvas[1])
        )
