...
class ToolExtractDrills(AppTool):
    ...
    def set_tool_ui(self):
        ...
        self.ui.circular_ring_entry.set_value(
            float(self.app.defaults["tools_edrills_circular_ring"])
        )
        ...
        self.ui.rectangular_ring_entry.set_value(
            float(self.app.defaults["tools_edrills_rectangular_ring"])
        )
    ...
    def on_extract_drills_click(self):
        ...
        if mode == "fixed":
            tools = {1: {"tooldia": drill_dia, "drills": [], "slots": []}}
            ...
            if "solid_geometry" not in tools[1] or not tools[1]["solid_geometry"]:
                self.app.inform.emit(
                    "[WARNING_NOTCL] %s" % _("No drills extracted. Try different parameters.")
                )
                return
        elif mode == "ring":
            drills_found = set()
            for apid, apid_value in fcobj.apertures.items():
                ...
                if ap_type == "C":
                    ...
                    # if the height == width (float numbers so the reason for the following)
                    if abs(
                        float("%.*f" % (self.decimals, width))
                        - float("%.*f" % (self.decimals, height))
                    ) < (10**-self.decimals):
                ...
                for tool, tool_val in tools.items():
                    if abs(
                        float("%.*f" % (self.decimals, tool_val["tooldia"]))
                        - float("%.*f" % (self.decimals, dia))
                    ) < (10**-self.decimals):
                        tool_in_drills = tool
                ...
                for geo_el in apid_value["geometry"]:
                    if "follow" in geo_el and isinstance(geo_el["follow"], Point):
                        if tool_in_drills not in tools:
                            tools[tool_in_drills] = {"tooldia": dia, "drills": [], "slots": []}
                ...
                if tool_in_drills in tools:
                    if (
                        "solid_geometry" not in tools[tool_in_drills]
                        or not tools[tool_in_drills]["solid_geometry"]
                    ):
        ...
        def obj_init(obj_inst, app_inst):
            ...
            obj_inst.source_file = app_inst.f_handlers.export_excellon(
                obj_name=outname, local_use=obj_inst, filename=None, use_thread=False
            )
...
class ExtractDrillsUI:
    ...
    def __init__(self, layout, app):
        ...
        self.e_drills_button.setToolTip(_("Extract drills from a given Gerber file."))
