...
class ToolFiducials(AppTool):
    ...
    def set_tool_ui(self):
        ...
        self.ui.line_thickness_entry.set_value(
            float(self.app.defaults["tools_fiducials_line_thickness"])
        )
    ...
    def add_fiducials(self):
        ...
        if self.mode_method == "auto":
            ...
            self.click_points.append(
                (float("%.*f" % (self.decimals, x0)), float("%.*f" % (self.decimals, y0)))
            )
            self.ui.bottom_left_coords_entry.set_value(
                "(%.*f, %.*f)" % (self.decimals, x0, self.decimals, y0)
            )

            self.click_points.append(
                (float("%.*f" % (self.decimals, x1)), float("%.*f" % (self.decimals, y1)))
            )
            self.ui.top_right_coords_entry.set_value(
                "(%.*f, %.*f)" % (self.decimals, x1, self.decimals, y1)
            )

            if self.sec_position == "up":
                self.click_points.append(
                    (float("%.*f" % (self.decimals, x0)), float("%.*f" % (self.decimals, y1)))
                )
                self.ui.sec_points_coords_entry.set_value(
                    "(%.*f, %.*f)" % (self.decimals, x0, self.decimals, y1)
                )
            elif self.sec_position == "down":
                self.click_points.append(
                    (float("%.*f" % (self.decimals, x1)), float("%.*f" % (self.decimals, y0)))
                )
                self.ui.sec_points_coords_entry.set_value(
                    "(%.*f, %.*f)" % (self.decimals, x1, self.decimals, y0)
                )

            ret_val = self.add_fiducials_geo(
                self.click_points, g_obj=self.grb_object, fid_type=fid_type
            )
    ...
    def add_fiducials_geo(self, points_list, g_obj, fid_size=None, fid_type=None, line_size=None):
        ...
        line_thickness = (
            self.ui.line_thickness_entry.get_value() if line_size is None else line_size
        )
        ...
        if fid_type == "circular":
            ...
            if aperture_found:
                ...
                new_apertures[new_apid] = {"type": "C", "size": fid_size, "geometry": []}
            ...
            for pt in points_list:
                ...
                line_geo_hor = LineString(
                    [
                        (x - radius + (line_thickness / 2.0), y),
                        (x + radius - (line_thickness / 2.0), y),
                    ]
                )
                line_geo_vert = LineString(
                    [
                        (x, y - radius + (line_thickness / 2.0)),
                        (x, y + radius - (line_thickness / 2.0)),
                    ]
                )
            ...
            if aperture_found:
                ...
                new_apertures[new_apid] = {"type": "C", "size": line_thickness, "geometry": []}
            ...
            for pt in points_list:
                x = pt[0]
                y = pt[1]
                first_square = make_square_poly(
                    center_pt=(x - fid_size / 4, y + fid_size / 4), side_size=fid_size / 2
                )
                second_square = make_square_poly(
                    center_pt=(x + fid_size / 4, y - fid_size / 4), side_size=fid_size / 2
                )
            ...
            for ap_id, ap_val in g_obj.apertures.items():
                if ap_val["type"] == "R" and round(ap_val["size"], ndigits=self.decimals) == round(
                    new_ap_size, ndigits=self.decimals
                ):
                    aperture_found = ap_id
                    break
        ...
        if self.sec_position != "no":
            if len(self.click_points) == 2:
                ...
                self.app.inform.emit(
                    _("Click to add the second fiducial. Top Left or Bottom Right...")
                )
...
class FidoUI:
    ...
    def __init__(self, layout, app):
        ...
        self.line_thickness_label.setToolTip(_("Thickness of the line that makes the fiducial."))
        ...
        self.sm_object_label.setToolTip(_("The Soldermask Gerber object."))
