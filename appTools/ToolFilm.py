...
class Film(AppTool):
    ...
    def set_tool_ui(self):
        ...
        f_type = (
            self.app.defaults["tools_film_type"] if self.app.defaults["tools_film_type"] else "neg"
        )
        self.ui.film_type.set_value(str(f_type))
        self.ui.on_film_type(val=f_type)

        b_entry = (
            self.app.defaults["tools_film_boundary"]
            if self.app.defaults["tools_film_boundary"]
            else 0.0
        )
        self.ui.boundary_entry.set_value(float(b_entry))

        scale_stroke_width = (
            self.app.defaults["tools_film_scale_stroke"]
            if self.app.defaults["tools_film_scale_stroke"]
            else 0.0
        )
    ...
    def on_film_creation(self):
        ...
        if self.ui.film_type.get_value() == "pos":
            ...
            if self.ui.punch_cb.get_value() is False:
                self.generate_positive_normal_film(
                    name, boxname, factor=scale_stroke_width, ftype=file_type
                )
            else:
                self.generate_positive_punched_film(
                    name, boxname, source, factor=scale_stroke_width, ftype=file_type
                )
        else:
    ...
    def generate_positive_normal_film(self, name, boxname, factor, ftype="svg"):
        ...
        if ftype == "svg":
            filter_ext = "SVG Files (*.SVG);;" "All Files (*.*)"
        elif ftype == "png":
            filter_ext = "PNG Files (*.PNG);;" "All Files (*.*)"
        else:
            filter_ext = "PDF Files (*.PDF);;" "All Files (*.*)"
        ...
        try:
            ...
            filename, _f = FCFileSaveDialog.get_saved_filename(
                caption=_("Export positive film"), ext_filter=filter_ext
            )
        ...
        if str(filename) == "":
            ...
            self.export_positive(
                ...
                skew_factor_x=skew_factor_x,
                skew_factor_y=skew_factor_y,
                skew_reference=skew_reference,
                mirror=mirror,
                pagesize_val=pagesize,
    ...
    def generate_positive_punched_film(self, name, boxname, source, factor, ftype="svg"):
        ...
        if source == "exc":
            log.debug(
                "ToolFilm.Film.generate_positive_punched_film() with Excellon source started ..."
            )
            ...
            punched_solid_geometry = MultiPolygon(film_obj.solid_geometry).difference(
                exc_solid_geometry
            )
            ...
            log.debug(
                "ToolFilm.Film.generate_positive_punched_film() with Pad center source started ..."
            )
            ...
            for apid in film_obj.apertures:
                if film_obj.apertures[apid]["type"] == "C":
                    ...
                    if punch_size >= float(
                        film_obj.apertures[apid]["width"]
                    ) or punch_size >= float(film_obj.apertures[apid]["height"]):
                        self.app.inform.emit(
                            "[ERROR_NOTCL] %s"
                            % _(
                                "Failed. Punch hole size "
            ...
            if punched_solid_geometry == temp_solid_geometry:
                self.app.inform.emit(
                    "[WARNING_NOTCL] %s"
                    % _(
                        "Failed. The new object geometry "
    ...
    def generate_negative_film(self, name, boxname, factor, ftype="svg"):
        ...
        try:
            ...
            filename, _f = FCFileSaveDialog.get_saved_filename(
                caption=_("Export negative film"), ext_filter=filter_ext
            )
    ...
    def export_negative(
        ...
        if box is None:
            self.app.inform.emit(
                "[WARNING_NOTCL] %s: %s" % (_("No object Box. Using instead"), obj)
            )
            box = obj
        ...
        def make_negative_film(scale_factor_x, scale_factor_y):
            ...
            if scale_factor_x and not scale_factor_y:
                transformed_box_geo = affinity.scale(
                    transformed_box_geo, scale_factor_x, 1.0, origin=tr_scale_reference
                )
            elif not scale_factor_x and scale_factor_y:
                transformed_box_geo = affinity.scale(
                    transformed_box_geo, 1.0, scale_factor_y, origin=tr_scale_reference
                )
            elif scale_factor_x and scale_factor_y:
                transformed_box_geo = affinity.scale(
                    transformed_box_geo, scale_factor_x, scale_factor_y, origin=tr_scale_reference
                )

            if skew_factor_x and not skew_factor_y:
                transformed_box_geo = affinity.skew(
                    transformed_box_geo, skew_factor_x, 0.0, origin=skew_ref
                )
            elif not skew_factor_x and skew_factor_y:
                transformed_box_geo = affinity.skew(
                    transformed_box_geo, 0.0, skew_factor_y, origin=skew_ref
                )
            elif skew_factor_x and skew_factor_y:
                transformed_box_geo = affinity.skew(
                    transformed_box_geo, skew_factor_x, skew_factor_y, origin=skew_ref
                )
            ...
            exported_svg = obj.export_svg(
                scale_stroke_factor=scale_stroke_factor,
                scale_factor_x=scale_factor_x,
                scale_factor_y=scale_factor_y,
                skew_factor_x=skew_factor_x,
                skew_factor_y=skew_factor_y,
    ...
    def export_positive(
        ...
        def make_positive_film(
            p_size, orientation, color, transparency_level, scale_factor_x, scale_factor_y
        ):
...
class FilmUI:
    ...
    def __init__(self, layout, app):
        ...
        self.tf_type_obj_combo = RadioSet(
            [{"label": _("Gerber"), "value": "grb"}, {"label": _("Geometry"), "value": "geo"}]
        )
        ...
        self.tf_object_combo.setRootModelIndex(
            self.app.collection.index(0, 0, QtCore.QModelIndex())
        )
        ...
        self.tf_type_box_combo = RadioSet(
            [{"label": _("Gerber"), "value": "grb"}, {"label": _("Geometry"), "value": "geo"}]
        )
        ...
        self.ois_scale = OptionalHideInputSection(
            self.film_scale_cb,
            [
        ...
        self.film_skew_reference = RadioSet(
            [
        ...
        self.ois_skew = OptionalHideInputSection(
            self.film_skew_cb,
            [
        ...
        self.film_mirror_axis = RadioSet(
            [
        ...
        self.ois_mirror = OptionalHideInputSection(
            self.film_mirror_cb, [self.film_mirror_axis_label, self.film_mirror_axis]
        )
        ...
        # Film Type
        self.film_type = RadioSet(
            [{"label": _("Positive"), "value": "pos"}, {"label": _("Negative"), "value": "neg"}],
            stretch=False,
        )
        ...
        self.source_punch = RadioSet(
            [{"label": _("Excellon"), "value": "exc"}, {"label": _("Pad center"), "value": "pad"}],
            stretch=False,
        )
        ...
        self.punch_size_label.setToolTip(
            _("The value here will control how big is the punch hole in the pads.")
        )
        ...
        # File type
        self.file_type_radio = RadioSet(
            [
                {"label": _("SVG"), "value": "svg"},
        ...
        self.orientation_radio = RadioSet(
            [
    ...
    def on_film_type(self, val):
        ...
        if type_of_film == "neg":
            ...
            self.punch_cb.set_value(
                False
            )  # required so the self.punch_frame it's hidden also by the signal emitted
            self.punch_cb.hide()
        else:
            self.boundary_label.hide()
            self.boundary_entry.hide()
    ...
    def on_punch_source(self, val):
        ...
        if val == "pad" and self.tf_type_obj_combo.get_value() == "geo":
            ...
            self.app.inform.emit(
                "[WARNING_NOTCL] %s"
                % _(
                    "Using the Pad center does not work on Geometry objects. "
