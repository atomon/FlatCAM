...
class CutOut(AppTool):
    ...
    def on_type_obj_changed(self, val):
        ...
        self.ui.obj_combo.setRootModelIndex(
    ...
    def on_cutout_tool_add_from_db_executed(self, tool):
        ...
        tool_from_db["data"]["tools_cutout_depthperpass"] = deepcopy(
            tool_from_db["data"]["depthperpass"]
        )
    ...
    def on_freeform_cutout(self):
        ...
        try:
            ...
            self.app.inform.emit(
                "[WARNING_NOTCL] %s" % _("Number of gaps value is missing. Add it and retry.")
            )
            return
        ...
        def cutout_handler(geom, gapsize):
            ...
            if gaps == "None":
                ...
                if gaps == "8" or gaps == "2LR":
                    ...
                    r_temp_geo.append(
                        self.intersect_geo(
                            initial_geo, box(points[0], points[1], points[2], points[3])
                        )
                    )
                ...
                if gaps == "4" or gaps == "LR":
                    points = (xxmin - gapsize, py - gapsize, xxmax + gapsize, py + gapsize)
                    geom = self.subtract_poly_from_geo(geom, points)
                    r_temp_geo.append(
                        self.intersect_geo(
                            initial_geo, box(points[0], points[1], points[2], points[3])
                ...
                if gaps == "4" or gaps == "TB":
                    points = (px - gapsize, yymin - gapsize, px + gapsize, yymax + gapsize)
                    geom = self.subtract_poly_from_geo(geom, points)
                    r_temp_geo.append(
                        self.intersect_geo(
                            initial_geo, box(points[0], points[1], points[2], points[3])
            ...
            def job_thread(app_obj):
                ...
                def geo_init(geo_obj, app_object):
                    ...
                    if not gaps_solid_geo:
                        ...
                        geo_obj.tools[9999]["data"][
                            "depthperpass"
                        ] = self.ui.maxdepth_entry.get_value()
                ...
                def excellon_init(exc_obj, app_o):
                    ...
                    tools = {1: {"tooldia": mb_dia, "drills": holes, "solid_geometry": []}}

                    exc_obj.tools = tools
                    exc_obj.create_geometry()
                    exc_obj.source_file = app_o.f_handlers.export_excellon(
                        obj_name=exc_obj.options["name"],
                        local_use=exc_obj,
                        filename=None,
                        use_thread=False,
                    )
    ...
    def on_rectangular_cutout(self):
        ...
        def cutout_rect_handler(geom, gapsize, xmin, ymin, xmax, ymax):
            ...
            if gaps == "None":
                ...
                if gaps == "4" or gaps == "LR":
                    points = (xmin - gapsize, py - gapsize, xmax + gapsize, py + gapsize)
                    geom = self.subtract_poly_from_geo(geom, points)

                if gaps == "4" or gaps == "TB":
                    points = (px - gapsize, ymin - gapsize, px + gapsize, ymax + gapsize)
                    geom = self.subtract_poly_from_geo(geom, points)
        ...
        with self.app.proc_container.new("Generating Cutout ..."):
            ...
            def job_thread(app_obj):
                ...
                if kind == "single":
                    ...
                    if cutout_obj.kind == "geometry":
                        ...
                        for geom_struct in object_geo:
                            ...
                            c_geo = cutout_rect_handler(
                                geom_struct, gapsize, xmin, ymin, xmax, ymax
                            )
                            solid_geo += c_geo
                            if gap_type == "bt" and thin_entry != 0:
                                try:
                    ...
                    if kind == "single":
                        ...
                        if cutout_obj.kind == "geometry":
                            ...
                            for mb_geom_struct in mb_object_geo:
                                ...
                                c_geo = cutout_rect_handler(
                                    mb_geom_struct, gapsize, xmin, ymin, xmax, ymax
                                )
                                solid_geo += c_geo
                            ...
                            msg = "[WARNING_NOTCL] %s" % _(
                                "Rectangular cutout with negative margin is not possible."
                            )
                ...
                try:
                    ...
                    # cutout_obj.plot(plot_tool=1)
                    app_obj.inform.emit(
                        "[success] %s" % _("Rectangular CutOut operation finished.")
                    )
                    # self.app.ui.notebook.setCurrentWidget(self.app.ui.project_tab)
                    app_obj.should_we_save = True
                except Exception as ee:
    ...
    def on_manual_gap_click(self):
        ...
        self.app.inform.emit(
            _("Click on the selected geometry object perimeter to create a bridge gap ...")
        )
        ...
        if self.ui.gaptype_radio.get_value() == "mb":
            ...
            self.mb_manual_solid_geo = self.flatten(
                unary_union(self.manual_solid_geo).buffer(b_dia).interiors
            )
    ...
    def on_manual_cutout(self, click_pos):
        ...
        if self.man_cutout_obj is None:
            msg = "[ERROR_NOTCL] %s: %s" % (
                _("Geometry object for manual cutout not found"),
                self.man_cutout_obj,
            )
            self.app.inform.emit(msg)
            return
        ...
        try:
            ...
            self.man_cutout_obj.tools[1]["data"]["name"] = (
                self.man_cutout_obj.options["name"] + "_cutout"
            )
            ...
            self.man_cutout_obj.tools[1]["data"][
                "depthperpass"
            ] = self.ui.maxdepth_entry.get_value()
        except KeyError:
        ...
        if gaps_solid_geo:
            if 9999 not in self.man_cutout_obj.tools:
                self.man_cutout_obj.tools.update({9999: self.cut_tool_dict})
                self.man_cutout_obj.tools[9999]["tooldia"] = str(dia)
                self.man_cutout_obj.tools[9999]["solid_geometry"] = [gaps_solid_geo]

                self.man_cutout_obj.tools[9999]["data"]["name"] = (
                    self.man_cutout_obj.options["name"] + "_cutout"
                )
                self.man_cutout_obj.tools[9999]["data"][
                    "cutz"
                ] = self.ui.thin_depth_entry.get_value()
        ...
        self.app.inform.emit(
            "%s" % _("Added manual Bridge Gap. Left click to add another or right click to finish.")
        )
    ...
    def on_manual_geo(self):
        ...
        def geo_init(geo_obj, app_obj):
            ...
            if convex_box:
                ...
                if (
                    isinstance(geo_union, Polygon)
                    or (isinstance(geo_union, list) and len(geo_union) == 1)
                    or (isinstance(geo_union, MultiPolygon) and len(geo_union) == 1)
                ):
                    ...
                    app_obj.inform.emit(
                        "[ERROR_NOTCL] %s: %s" % (_("Geometry not supported"), type(geo_union))
                    )
                    return "fail"
            else:
                geo = geo_union
            ...
            geo_obj.tools.update({1: self.cut_tool_dict})
            geo_obj.tools[1]["tooldia"] = str(dia)
            geo_obj.tools[1]["solid_geometry"] = geo_obj.solid_geometry
    ...
    def subtract_geo(target_geo, subtractor):
        ...
        for target in flat_geometry:
            if (
                ...
                or isinstance(target, LinearRing)
...
class CutoutUI:
    ...
    def __init__(self, layout, app):
        ...
        title_param_label.setToolTip(_("This section handle creation of automatic bridge gaps."))
        ...
        self.ff_cutout_object_btn.setIcon(
            QtGui.QIcon(self.app.resource_location + "/irregular32.png")
        )
        ...
        self.rect_cutout_object_btn.setIcon(
        ...
        big_cursor_label.setToolTip(_("Use a big cursor when adding manual gaps."))
        ...
        self.man_geo_creation_btn.setIcon(
            QtGui.QIcon(self.app.resource_location + "/rectangle32.png")
        )
        ...
        self.man_object_combo.setRootModelIndex(
        ...
        self.man_object_label.setToolTip(_("Geometry object used to create the manual cutout."))
