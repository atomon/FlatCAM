...
class AlignObjects(AppTool):
    ...
    def on_type_aligner_changed(self, val):
        obj_type = {"grb": 0, "exc": 1}[val]
        self.ui.aligner_object_combo.setRootModelIndex(
            self.app.collection.index(obj_type, 0, QtCore.QModelIndex())
        )
    ...
    def on_align(self):
        ...
        obj_model_index = self.app.collection.index(
            obj_sel_index, 0, self.ui.object_combo.rootModelIndex()
        )
        try:
            self.aligned_obj = obj_model_index.internalPointer().obj
        except AttributeError:
            self.app.inform.emit(
                "[WARNING_NOTCL] %s" % _("There is no aligned FlatCAM object selected...")
            )
            return
        ...
        try:
            ...
            self.app.inform.emit(
                "[WARNING_NOTCL] %s" % _("There is no aligner FlatCAM object selected...")
            )
            return
    ...
    def check_points(self):
        if len(self.clicked_points) == 1:
            self.app.inform.emit(
                ...
                % (
                    _("First Point"),
                    _("Click on the DESTINATION point ..."),
                    _("Or right click to cancel."),
                )
            )
    ...
    def set_color(self):
        ...
        self.target_obj.shapes.redraw(update_colors=(new_color, new_line_color))
...
class AlignUI:
    ...
    def __init__(self, layout, app):
        ...
        self.object_combo.setToolTip(_("Object to be aligned."))
        ...
        self.aligner_object_combo.setRootModelIndex(
            self.app.collection.index(0, 0, QtCore.QModelIndex())
        )
        self.aligner_object_combo.is_last = True

        self.aligner_object_combo.setToolTip(_("Object to be aligned to. Aligner."))
        ...
        self.a_type_radio = RadioSet(
            [{"label": _("Single Point"), "value": "sp"}, {"label": _("Dual Point"), "value": "dp"}]
        )
