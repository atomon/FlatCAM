...
class ToolPunchGerber(AppTool):
    ...
    def __init__(self, app):
        ...
        self.ui.circular_cb.stateChanged.connect(
            lambda state: (
                self.ui.circular_ring_entry.setDisabled(False)
                if state
                else self.ui.circular_ring_entry.setDisabled(True)
            )
        )

        self.ui.oblong_cb.stateChanged.connect(
            lambda state: (
                self.ui.oblong_ring_entry.setDisabled(False)
                if state
                else self.ui.oblong_ring_entry.setDisabled(True)
            )
        )

        self.ui.square_cb.stateChanged.connect(
            lambda state: (
                self.ui.square_ring_entry.setDisabled(False)
                if state
                else self.ui.square_ring_entry.setDisabled(True)
            )
        )

        self.ui.rectangular_cb.stateChanged.connect(
            lambda state: (
                self.ui.rectangular_ring_entry.setDisabled(False)
                if state
                else self.ui.rectangular_ring_entry.setDisabled(True)
            )
        )

        self.ui.other_cb.stateChanged.connect(
            lambda state: (
                self.ui.other_ring_entry.setDisabled(False)
                if state
                else self.ui.other_ring_entry.setDisabled(True)
            )
        )
    ...
    def set_tool_ui(self):
        ...
        self.ui.rectangular_ring_entry.set_value(
            float(self.app.defaults["tools_punch_rectangular_ring"])
        )
        ...
        for ap_code in sorted_apertures:
            ...
            try:
                if obj.apertures[ap_code]["size"] is not None:
                    size_val = self.app.dec_format(
                        float(obj.apertures[ap_code]["size"]), self.decimals
                    )
    ...
    def on_excellon_method(self, grb_obj, outname):
                            ...
                            if "drills" in exc_obj.tools[tool]:
                                for drill_pt in exc_obj.tools[tool]["drills"]:
                                    ...
                                    if drill_pt.within(elem["solid"]):
                                        ...
                                        holes_apertures[clear_apid_size]["geometry"].append(
                                            deepcopy(geo_elem)
                                        )
        ...
        def init_func(new_obj, app_obj):
            ...
            new_obj.source_file = app_obj.f_handlers.export_gerber(
                obj_name=outname, filename=None, local_use=new_obj, use_thread=False
            )
    ...
    def on_fixed_method(self, grb_obj, outname):
        ...
        if punch_size == 0.0:
            self.app.inform.emit(
                "[WARNING_NOTCL] %s" % _("The value of the fixed diameter is 0.0. Aborting.")
            )
            return "fail"

        fail_msg = _(
            "Failed. Punch hole size is bigger than" " some of the apertures in the Gerber object."
        )
        ...
        for apid in grb_obj.apertures:
            if apid in sel_apid:
                if grb_obj.apertures[apid]["type"] == "C" and self.ui.circular_cb.get_value():
                    ...
                    if (
                        round(float(grb_obj.apertures[apid]["width"]), self.decimals)
                        == round(float(grb_obj.apertures[apid]["height"]), self.decimals)
                        and self.ui.square_cb.get_value()
                    ):
                        for elem in grb_obj.apertures[apid]["geometry"]:
                            if "follow" in elem:
                                if isinstance(elem["follow"], Point):
                                    if punch_size >= float(
                                        grb_obj.apertures[apid]["width"]
                                    ) or punch_size >= float(grb_obj.apertures[apid]["height"]):
                                        self.app.inform.emit("[ERROR_NOTCL] %s" % fail_msg)
                                        return "fail"
                                    punching_geo.append(elem["follow"].buffer(punch_size / 2))
                    elif (
                        round(float(grb_obj.apertures[apid]["width"]), self.decimals)
                        != round(float(grb_obj.apertures[apid]["height"]), self.decimals)
                        and self.ui.rectangular_cb.get_value()
                    ):
                        for elem in grb_obj.apertures[apid]["geometry"]:
                            if "follow" in elem:
                                if isinstance(elem["follow"], Point):
                    ...
                    grb_obj.apertures[apid]["type"] not in ["C", "R", "O"]
                    and self.ui.other_cb.get_value()
                ):
                    for elem in grb_obj.apertures[apid]["geometry"]:
                        if "follow" in elem:
                            if isinstance(elem["follow"], Point):
    ...
    def on_mark_cb_click_table(self):
        ...
        if self.ui.apertures_table.cellWidget(cw_row, 3).isChecked():
            ...
            grb_obj.plot_aperture(
                color=self.app.defaults["global_sel_draw_color"] + "AA",
...
class PunchUI:
    ...
    def __init__(self, layout, app):
        ...
        self.circular_cb.setToolTip(_("Process Circular Pads."))

        pad_grid.addWidget(self.circular_cb, 1, 0)

        # Oblong Aperture Selection
        self.oblong_cb = FCCheckBox("%s" % _("Oblong"))
        self.oblong_cb.setToolTip(_("Process Oblong Pads."))

        pad_grid.addWidget(self.oblong_cb, 2, 0)

        # Square Aperture Selection
        self.square_cb = FCCheckBox("%s" % _("Square"))
        self.square_cb.setToolTip(_("Process Square Pads."))

        pad_grid.addWidget(self.square_cb, 3, 0)

        # Rectangular Aperture Selection
        self.rectangular_cb = FCCheckBox("%s" % _("Rectangular"))
        self.rectangular_cb.setToolTip(_("Process Rectangular Pads."))

        pad_grid.addWidget(self.rectangular_cb, 4, 0)

        # Others type of Apertures Selection
        self.other_cb = FCCheckBox("%s" % _("Others"))
        self.other_cb.setToolTip(_("Process pads not in the categories above."))
        ...
        self.apertures_table.horizontalHeaderItem(0).setToolTip(_("Aperture Code"))
        self.apertures_table.horizontalHeaderItem(1).setToolTip(
            _("Type of aperture: circular, rectangle, macros etc")
        )
        self.apertures_table.horizontalHeaderItem(2).setToolTip(_("Aperture Size:"))
        ...
        self.dia_label.setToolTip(_("Fixed hole diameter."))
        ...
        self.circular_ring_label.setToolTip(_("The size of annular ring for circular pads."))
        ...
        self.oblong_ring_label.setToolTip(_("The size of annular ring for oblong pads."))
        ...
        self.square_ring_label.setToolTip(_("The size of annular ring for square pads."))
        ...
        self.rectangular_ring_label.setToolTip(_("The size of annular ring for rectangular pads."))
        ...
        self.other_ring_label.setToolTip(_("The size of annular ring for other pads."))
