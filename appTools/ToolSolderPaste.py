...
class SolderPaste(AppTool):
    ...
    def __init__(self, app):
        ...
        self.combo_context_del_action = QtWidgets.QAction(
            QtGui.QIcon(self.app.resource_location + "/trash16.png"), _("Delete Object")
        )
    ...
    def set_tool_ui(self):
        ...
        self.ui.tools_table.addContextMenu(
            _("Add"),
            lambda: self.on_tool_add(dia=None, muted=None),
        ...
        try:
            dias = [
                float(eval(dia))
                for dia in self.app.defaults["tools_solderpaste_tools"].split(",")
                if dia != ""
            ]
        except Exception:
            log.error(
                "At least one Nozzle tool diameter needed. "
    ...
    def ui_connect(self):
        ...
        for i in range(self.ui.gcode_form_layout.count()):
            if isinstance(self.ui.gcode_form_layout.itemAt(i).widget(), FCComboBox):
                self.ui.gcode_form_layout.itemAt(i).widget().currentIndexChanged.connect(
                    self.read_form_to_tooldata
                )
            if isinstance(self.ui.gcode_form_layout.itemAt(i).widget(), FCEntry):
                self.ui.gcode_form_layout.itemAt(i).widget().editingFinished.connect(
                    self.read_form_to_tooldata
                )
    ...
    def read_form_to_tooldata(self, tooluid=None):
        ...
        for key in self.form_fields:
            self.tooltable_tools[uid]["data"].update({key: self.form_fields[key].get_value()})
    ...
    def on_create_geo_click(self):
        ...
        if name == "":
            self.app.inform.emit(
                "[WARNING_NOTCL] %s" % _("No SolderPaste mask Gerber object loaded.")
            )
            return
    ...
    def on_create_geo(self, name, work_object, use_thread=True):
        ...
        def flatten(geometry=None, reset=True, pathonly=False):
            ...
            try:
                for geo in geometry:
                    if geo is not None:
                        flatten(geometry=geo, reset=False, pathonly=pathonly)
        ...
        def geo_init(geo_obj, app_obj):
            ...
            for tool in sorted_tools:
                ...
                for g in work_geo:
                    ...
                    diag_1_intersect = LineString([(x_min, y_min), (x_max, y_max)]).intersection(
                        poly
                    )
                    diag_2_intersect = LineString([(x_min, y_max), (x_max, y_min)]).intersection(
                        poly
                    )
                ...
                if not work_geo:
                    ...
                    app_obj.inform.emit(
                        "[success] %s..." % _("Solder Paste geometry generated successfully")
                    )
                    return
            ...
            if work_geo:
                app_obj.inform.emit(
                    "[WARNING_NOTCL] %s"
                    % _("Some or all pads have no solder " "due of inadequate nozzle diameters...")
                )
                return "fail"
    ...
    def on_create_gcode_click(self):
        ...
        if obj.special_group != "solder_paste_tool":
            self.app.inform.emit(
                "[WARNING_NOTCL] %s"
                % _("This Geometry can't be processed. " "NOT a solder_paste_tool geometry.")
            )
            return
    ...
    def on_create_gcode(self, name, workobject, use_thread=True):
        ...
        def job_init(job_obj):
            ...
            for tooluid_key, tooluid_value in obj.tools.items():
                ...
                tool_cnc_dict["solid_geometry"] = unary_union(
                    [geo["geom"] for geo in tool_cnc_dict["gcode_parsed"]]
                )

                # tell gcode_parse from which point to start drawing the lines depending on what kind of
                # object is the source of gcode
                job_obj.toolchange_xy_type = "geometry"
                job_obj.cnc_tools.update({tooluid_key: deepcopy(tool_cnc_dict)})
                tool_cnc_dict.clear()
    ...
    def on_view_gcode(self):
        ...
        try:
            if obj.special_group != "solder_paste_tool":
                self.app.inform.emit(
                    "[WARNING_NOTCL] %s"
                    % _(
                        "This CNCJob object can't be processed. "
        ...
        gcode += (
            "(Type: "
            + "G-code from "
            + str(obj.options["type"])
            + " for Solder Paste dispenser"
            + ")\n"
        )
    ...
    def on_save_gcode(self):
        ...
        try:
            ...
            filename, _f = FCFileSaveDialog.get_saved_filename(
                caption=_("Export GCode ..."), directory=dir_file_to_save, ext_filter=_filter_
            )
        except TypeError:
            filename, _f = FCFileSaveDialog.get_saved_filename(
        ...
        self.app.inform.emit(
            "[success] %s: %s" % (_("Solder paste dispenser GCode file saved to"), filename)
        )

    def reset_fields(self):
        self.ui.obj_combo.setRootModelIndex(self.app.collection.index(0, 0, QtCore.QModelIndex()))
        self.ui.geo_obj_combo.setRootModelIndex(
            self.app.collection.index(2, 0, QtCore.QModelIndex())
        )
        self.ui.cnc_obj_combo.setRootModelIndex(
            self.app.collection.index(3, 0, QtCore.QModelIndex())
        )
...
class SolderUI:
    ...
    def __init__(self, layout, app, solder_class):
        ...
        self.addtool_entry_lbl.setToolTip(_("Diameter for the new tool to add in the Tool Table"))
        ...
        self.z_start_label.setToolTip(_("The height (Z) when solder paste dispensing starts."))
        ...
        self.z_dispense_label.setToolTip(_("The height (Z) when doing solder paste dispensing."))
        ...
        self.z_stop_label.setToolTip(_("The height (Z) when solder paste dispensing stops."))
        ...
        self.z_toolchange_label.setToolTip(_("The height (Z) for tool (nozzle) change."))
        ...
        self.frxy_label.setToolTip(_("Feedrate (speed) while moving on the X-Y plane."))
        ...
        self.frz_label.setToolTip(_("Feedrate (speed) while moving vertically\n" "(on Z plane)."))
        ...
        self.dwellfwd_label.setToolTip(_("Pause after solder dispensing."))
        ...
        pp_label.setToolTip(_("Files that control the GCode generation."))
        ...
        self.soldergeo_btn.setToolTip(_("Generate solder paste dispensing geometry."))
