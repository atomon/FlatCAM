...
class NonCopperClear(AppTool, Gerber):
    ...
    def on_type_obj_index_changed(self, val):
        ...
        self.ui.object_combo.obj_type = {"gerber": "Gerber", "geometry": "Geometry"}[
    ...
    def on_apply_param_to_all_clicked(self):
        if self.ui.tools_table.rowCount() == 0:
            # there is no tool in tool table so we can't save the GUI elements values to storage
            log.debug(
                "NonCopperClear.on_apply_param_to_all_clicked() --> no tool in Tools Table, aborting."
            )
            return
    ...
    def set_tool_ui(self):
        ...
        try:
            ...
            try:
                dias = [
                    float(eval(dia))
                    for dia in self.app.defaults["tools_ncc_tools"].split(",")
                    if dia != ""
                ]
            except AttributeError:
        ...
        for tool_sorted in sorted_tools:
            for tooluid_key, tooluid_value in self.ncc_tools.items():
                if float("%.*f" % (self.decimals, tooluid_value["tooldia"])) == tool_sorted:
                    ...
                    dia = QtWidgets.QTableWidgetItem(
                        "%.*f" % (self.decimals, tooluid_value["tooldia"])
                    )
    ...
    def on_tool_edit(self, item):
        ...
        if new_tool_dia not in tool_dias:
            self.ncc_tools[editeduid]["tooldia"] = deepcopy(
    ...
    def on_ncc_click(self):
        ...
        self.circle_steps = (
            int(self.app.defaults["gerber_circle_steps"])
            if obj_type == "gerber"
            else int(self.app.defaults["geometry_circle_steps"])
        )
        ...
        if len(sel_rows) > 0:
            for row in sel_rows:
                try:
                    ...
                    try:
                        self.tooldia = float(
                            self.ui.tools_table.item(row, 1).text().replace(",", ".")
                        )
                    except ValueError:
                        self.app.inform.emit(
                            "[ERROR_NOTCL] %s" % _("Wrong value format entered, use a number.")
        ...
        if self.select_method == 0:  # Itself
            ...
            try:
                ...
                self.app.inform.emit(
                    "[ERROR_NOTCL] %s: %s" % (_("Could not retrieve object"), self.bound_obj_name)
                )
                return "Could not retrieve object: %s with error: %s" % (
                    self.bound_obj_name,
                    str(e),
                )
        ...
        if key == QtCore.Qt.Key_Escape or key == "Escape":
            ...
            if self.area_sel_disconnect_flag is True:
                ...
                try:
                    ...
                    if self.app.is_legacy is False:
                        self.app.plotcanvas.graph_event_disconnect(
                            "mouse_release", self.on_single_poly_mouse_release
                        )
                        self.app.plotcanvas.graph_event_disconnect("key_press", self.on_key_press)
                    else:
                        self.app.plotcanvas.graph_event_disconnect(self.mr)
        ...
        if ncc_select == 0:  # _('Itself')
            ...
            try:
                if isinstance(geo_n, MultiPolygon):
                    ...
                    or (isinstance(geo_n, list) and len(geo_n) == 1)
            ...
            if box_kind == "geometry":
                ...
                self.app.inform.emit(
                    "[ERROR_NOTCL] %s" % _("The reference object type is not supported.")
                )
                return "fail"
    ...
    def apply_margin_to_bounding_box(self, bbox, box_kind, ncc_select, ncc_margin):
        ...
        if ncc_select == 0:  # _('Itself')
            try:
                new_bounding_box = bbox.buffer(
                    distance=ncc_margin, join_style=base.JOIN_STYLE.mitre
                )
            except Exception as e:
            ...
            for poly in bbox:
                if self.app.abort_flag:
                    # graceful abort requested by the user
                    raise grace
                geo_buff_list.append(
                    poly.buffer(distance=ncc_margin, join_style=base.JOIN_STYLE.mitre)
                )
    ...
    def get_tool_empty_area(
        ...
        if work_geo:
            ...
            if empty == "fail" or empty.is_empty:
                msg = "[ERROR_NOTCL] %s" % _(
                    "Could not get the extent of the area to be non copper cleared."
                )
            ...
            for tool_iso in isotooldia:
                ...
                if ncc_margin < tool_iso:
                    self.app.inform.emit(
                        "[WARNING_NOTCL] %s"
                        % _(
                            "Isolation geometry is broken. Margin is less "
                ...
                for k, v in tools_storage.items():
                    if float("%.*f" % (self.decimals, v["tooldia"])) == float(
                        "%.*f" % (self.decimals, tool_iso)
                    ):
                        current_uid = int(k)
    ...
    def clear_polygon_worker(
        self, pol, tooldia, ncc_method, ncc_overlap, ncc_connect, ncc_contour, prog_plot
    ):
            ...
            try:
                ...
                if cp and cp.objects:
                    ...
                    if cp and cp.objects:
                        ...
                        self.app.inform.emit(
                            _("Failed. Clearing the polygon with the method: standard.")
                        )
                        cp = self.clear_polygon(
                            pol,
                            tooldia,
        ...
        if cp and cp.objects:
            ...
            coords = (pt.x, pt.y)
            self.app.inform_shell.emit(
                "%s %s" % (_("Polygon could not be cleared. Location:"), str(coords))
            )
            return None
    ...
    def clear_copper(
        ...
        if not sorted_clear_tools:
            self.app.inform.emit(
                "[ERROR_NOTCL] %s"
                % _(
                    "There is no copper clearing tool in the selection "
        ...
        def gen_clear_area(geo_obj, app_obj):
            ...
            self.app.inform.emit(
                _("NCC Tool. Finished non-copper polygons. Normal copper clearing task started.")
            )
            ...
            if ncc_select == 2:  # Reference Object
                ...
                bbox_geo, bbox_kind = self.calculate_bounding_box(
                    ncc_obj=ncc_obj, ncc_select=ncc_select
                )

            if bbox_geo is None and bbox_kind is None:
                self.app.inform.emit(
                    "[ERROR_NOTCL] %s" % _("NCC Tool failed creating bounding box.")
                )
                return "fail"

            # Bounding box for current tool
            ncc_margin = self.ui.ncc_margin_entry.get_value()
            bbox = self.apply_margin_to_bounding_box(
                bbox=bbox_geo, box_kind=bbox_kind, ncc_select=ncc_select, ncc_margin=ncc_margin
            )
            ...
            for tool in sorted_clear_tools:
                ...
                app_obj.inform.emit(
                    "[success] %s = %s%s %s"
                    % (
                        _("NCC Tool clearing with tool diameter"),
                        str(tool),
                        units.lower(),
                        _("started."),
                    )
                ...
                # Area to clear
                area, warning_flag = self.get_tool_empty_area(
                    name=name,
                    ncc_obj=ncc_obj,
                    geo_obj=geo_obj,
                ...
                if area.geoms:
                    if len(area.geoms) > 0:
                                ...
                                try:
                                    for pol in p:
                                        ...
                                        if pol is not None and isinstance(pol, Polygon):
                                            ...
                                            log.warning(
                                                "Expected geo is a Polygon. Instead got a %s"
                                                % str(type(pol))
                                            )
                                except TypeError:
                        ...
                        if cleared_geo:
                            ...
                            for k, v in tools_storage.items():
                                if (
                                    self.app.dec_format(v["tooldia"], self.decimals)
                                    == formatted_tool
                                ):
                                    current_uid = int(k)
            ...
            for uid, uid_val in list(tools_storage.items()):
                try:
                    ...
                    if not uid_val["solid_geometry"]:
                        ...
                        self.app.inform.emit(msg)
                        log.debug(
                            "Empty geometry for tool: %s with diameter: %s"
                            % (str(uid), str(uid_val["tooldia"]))
                        )
        ...
        def gen_clear_area_rest(geo_obj, app_obj):
            ...
            ncc_margin = self.ui.ncc_margin_entry.get_value()
            bbox = self.apply_margin_to_bounding_box(
                bbox=env_obj, box_kind=box_obj_kind, ncc_select=ncc_select, ncc_margin=ncc_margin
            )
            ...
            while sorted_clear_tools:
                ...
                tool_uid = 0  # find the current tool_uid
                for k, v in self.ncc_tools.items():
                    if self.app.dec_format(v["tooldia"], self.decimals) == self.app.dec_format(
                        tool, self.decimals
                    ):
                        tool_uid = int(k)
                        break
                ...
                if area.geoms and len(area.geoms) > 0:
                    ...
                    log.warning(
                        "Total number of polygons failed to be cleared: %s" % str(poly_failed)
                    )
                else:
            ...
            if geo_obj.tools:
                if warning_flag == 0:
                    self.app.inform.emit(
                        "[success] %s" % _("NCC Tool Rest Machining clear all done.")
                    )
                else:
                    self.app.inform.emit(
                        "[WARNING] %s: %s %s."
                        % (
                            _(
                                "NCC Tool Rest Machining clear all done but the copper features "
    ...
    def clear_copper_tcl(
        ...
        if ncc_select == 0:  # itself
            ...
            if ncc_sel_obj.kind == "geometry":
                ...
                bounding_box = unary_union(ncc_sel_obj.solid_geometry).convex_hull.intersection(
                    geo_n
                )
                ...
                if area.geoms:
                    if len(area.geoms) > 0:
                            ...
                            if p is not None and p.is_valid:
                                ...
                                try:
                                    for pol in p:
                                        if pol is not None and isinstance(pol, Polygon):
                                            ...
                                            if cp:
                                                ...
                                                log.warning(
                                                    "Polygon in MultiPolygon can not be cleared."
                                                )
                                        else:
                                            log.warning(
                                    ...
                                    if isinstance(p, Polygon):
                                        ...
                                        log.warning(
                                            "Geo can not be cleared because it is: %s"
                                            % str(type(p))
                                        )
                        ...
                        if cleared_geo:
                            ...
                            cleared = (
                                empty.buffer(-offset_a * (1 + overlap))
                                .buffer(-tool / 1.999999)
                                .buffer(tool / 1.999999)
                            )
                                    ...
                                    for poly_p in p:
                                        if poly_p is not None:
                                            ...
                                            try:
                                                ...
                                                log.warning(
                                                    "Polygon can't be cleared. %s" % str(eee)
                                                )
                                                # this polygon should be added to a list and then try clear it with
                                                # a smaller tool
            ...
            try:
                try:
                    ...
                    if isinstance(geom, Polygon) and geom is not None:
                        ...
                        log.debug(
                            "NonCopperClear.generate_envelope() Error --> Unexpected Geometry %s"
                            % type(geom)
                        )
            except Exception as e:
...
class NccUI:
    ...
    def __init__(self, layout, app):
        ...
        self.ncc_order_label.setToolTip(
            _(
        ...
        self.ncc_order_radio.setToolTip(
            _(
                "This set the way that the tools in the tools table are used.\n"
                "'No' --> means that the used order is the one in the tool table\n"
                "'Forward' --> means that the tools will be ordered from small to big\n"
        ...
        self.nccmarginlabel.setToolTip(_("Bounding box margin."))
        ...
        self.ois_ncc_offset = OptionalInputSection(
            self.ncc_choice_offset_cb, [self.ncc_offset_spinner]
        )
        ...
        self.rest_nccmarginlabel.setToolTip(_("Bounding box margin."))
        ...
        self.rest_ois_ncc_offset = OptionalInputSection(
            self.rest_ncc_choice_offset_cb, [self.rest_ncc_offset_spinner]
        )

        # Reference Selection Combo
        self.select_combo = FCComboBox2()
        self.select_combo.addItems([_("Itself"), _("Area Selection"), _("Reference Object")])
        ...
        self.grid3.addWidget(
            self.reference_combo_type_label,
            31,
            0,
        )
        ...
        self.generate_ncc_button.setIcon(
