...
class QRCode(AppTool):
    ...
    def set_tool_ui(self):
        ...
        self.ui.fill_color_button.setStyleSheet(
            "background-color:%s" % str(self.app.defaults["tools_qrcode_fill_color"])[:7]
        )

        self.ui.back_color_entry.set_value(self.app.defaults["tools_qrcode_back_color"])
        self.ui.back_color_button.setStyleSheet(
            "background-color:%s" % str(self.app.defaults["tools_qrcode_back_color"])[:7]
        )
    ...
    def execute(self):
        ...
        if text_data == "":
            self.app.inform.emit(
                "[ERROR_NOTCL] %s" % _("Cancelled. There is no QRCode Data in the text box.")
            )
            return
    ...
    def make(self, pos):
        ...
        if new_apid not in self.grb_object.apertures:
            ...
            self.grb_object.apertures[new_apid]["size"] = deepcopy(
                math.sqrt(box_size**2 + box_size**2)
            )

        if "0" not in self.grb_object.apertures:
            self.grb_object.apertures["0"] = {"type": "REG", "size": 0.0, "geometry": []}
        ...
        self.grb_object.source_file = self.app.f_handlers.export_gerber(
            obj_name=self.grb_object.options["name"],
            filename=None,
    ...
    def draw_utility_geo(self, pos):
        ...
        if len(self.qrcode_geometry) <= self.app.defaults["tools_qrcode_sel_limit"]:
            try:
                ...
                offset_geo.append(
                    translate(self.qrcode_utility_geometry.exterior, xoff=pos[0], yoff=pos[1])
                )
    ...
    def export_png_file(self):
        ...
        def job_thread_qr_png(app_obj, fname):
            ...
            img = qr.make_image(
                fill_color=self.ui.fill_color_entry.get_value(),
        ...
        try:
            ...
            filename, _f = FCFileSaveDialog.get_saved_filename(
                caption=_("Export PNG"), ext_filter=_filter
            )
...
class QRcodeUI:
    ...
    def __init__(self, layout, app):
        ...
        self.grbobj_label.setToolTip(_("Gerber Object to which the QRCode will be added."))
        ...
        self.text_label.setToolTip(_("QRCode Data. Alphanumeric text to be encoded in the QRCode."))
        self.text_data = FCTextArea()
        self.text_data.setPlaceholderText(_("Add here the text to be included in the QRCode..."))
        ...
        self.qrcode_label.setToolTip(_("The parameters used to shape the QRCode."))
        ...
        self.pol_radio = RadioSet(
            [{"label": _("Negative"), "value": "neg"}, {"label": _("Positive"), "value": "pos"}]
        )
        ...
        self.bb_radio = RadioSet(
            [{"label": _("Rounded"), "value": "r"}, {"label": _("Square"), "value": "s"}]
        )
        ...
        self.fill_color_label.setToolTip(_("Set the QRCode fill color (squares color)."))
        ...
        self.back_color_label.setToolTip(_("Set the QRCode background color."))
        ...
        self.export_svg_button.setToolTip(_("Export a SVG file with the QRCode content."))
        ...
        self.export_png_button.setToolTip(_("Export a PNG image file with the QRCode content."))
        ...
        self.qrcode_button.setToolTip(_("Create the QRCode object."))
