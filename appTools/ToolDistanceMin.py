...
class DistanceMin(AppTool):
    ...
    def activate_measure_tool(self):
        ...
        if self.app.call_source == "app":
            ...
            if len(selected_objs) != 2:
                ...
                if isinstance(selected_objs[0].solid_geometry, list):
                    try:
                        selected_objs[0].solid_geometry = MultiPolygon(
                            selected_objs[0].solid_geometry
                        )
                    except Exception:
                        selected_objs[0].solid_geometry = unary_union(
                            selected_objs[0].solid_geometry
                        )

                    try:
                        selected_objs[1].solid_geometry = MultiPolygon(
                            selected_objs[1].solid_geometry
                        )
                    except Exception:
                        selected_objs[1].solid_geometry = unary_union(
                            selected_objs[1].solid_geometry
                        )

                first_pos, last_pos = nearest_points(
                    selected_objs[0].solid_geometry, selected_objs[1].solid_geometry
                )
            ...
            if len(selected_objs) != 2:
                ...
                first_pos, last_pos = nearest_points(
                    selected_objs[0].geo["solid"], selected_objs[1].geo["solid"]
                )
        else:
            first_pos, last_pos = 0, 0

        self.ui.start_entry.set_value(
            "(%.*f, %.*f)" % (self.decimals, first_pos.x, self.decimals, first_pos.y)
        )
        self.ui.stop_entry.set_value(
            "(%.*f, %.*f)" % (self.decimals, last_pos.x, self.decimals, last_pos.y)
        )
        ...
        self.h_point = (
            min(first_pos.x, last_pos.x) + (abs(dx) / 2),
            min(first_pos.y, last_pos.y) + (abs(dy) / 2),
        )
        if d != 0:
        ...
        if d != 0:
            ...
            self.app.inform.emit(
                ...
                % (
                    ...
                    "(%.*f, %.*f)"
                    % (self.decimals, self.h_point[0], self.decimals, self.h_point[1]),
                )
            )
...
class DistMinUI:
    ...
    def __init__(self, layout, app):
        ...
        self.units_value = QtWidgets.QLabel(
        ...
        self.half_point_label.setToolTip(
            _("This is the middle point of the point to point Euclidean distance.")
        )

        self.start_entry = FCEntry()
        self.start_entry.setReadOnly(True)
        self.start_entry.setAlignment(QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter)
        self.start_entry.setToolTip(
            _(
                "This is first object point coordinates.\n"
                "This is the start point for measuring distance."
            )
        ...
        self.half_point_entry.setToolTip(
            _("This is the middle point of the point to point Euclidean distance.")
        )
