...
class Panelize(AppTool):
    ...
    def set_tool_ui(self):
        ...
        sp_c = (
            self.app.defaults["tools_panelize_spacing_columns"]
            if self.app.defaults["tools_panelize_spacing_columns"]
            else 0.0
        )
        self.ui.spacing_columns.set_value(float(sp_c))

        sp_r = (
            self.app.defaults["tools_panelize_spacing_rows"]
        ...
        rr = (
            self.app.defaults["tools_panelize_rows"]
        ...
        cc = (
            self.app.defaults["tools_panelize_columns"]
            if self.app.defaults["tools_panelize_columns"]
            else 0.0
        )
        self.ui.columns.set_value(int(cc))

        optimized_path_cb = (
            self.app.defaults["tools_panelize_optimization"]
        ...
        c_cb = (
            self.app.defaults["tools_panelize_constrain"]
            if self.app.defaults["tools_panelize_constrain"]
            else False
        )
        self.ui.constrain_cb.set_value(c_cb)

        x_w = (
            self.app.defaults["tools_panelize_constrainx"]
            if self.app.defaults["tools_panelize_constrainx"]
            else 0.0
        )
        self.ui.x_width_entry.set_value(float(x_w))

        y_w = (
            self.app.defaults["tools_panelize_constrainy"]
        ...
        panel_type = (
            self.app.defaults["tools_panelize_panel_type"]
            if self.app.defaults["tools_panelize_panel_type"]
            else "gerber"
        )
    ...
    def on_type_box_index_changed(self):
        ...
        self.ui.box_combo.obj_type = {_("Gerber"): "Gerber", _("Geometry"): "Geometry"}[
    ...
    def on_panelize(self):
        ...
        if panel_source_obj is None:
            self.app.inform.emit("[ERROR_NOTCL] %s: %s" % (_("Object not found"), panel_source_obj))
            return
        ...
        if box is None:
            self.app.inform.emit(
                "[WARNING_NOTCL] %s: %s" % (_("No object Box. Using instead"), panel_source_obj)
            )
            self.ui.reference_radio.set_value("bbox")
        ...
        def panelize_worker():
            if panel_source_obj is not None:
                ...
                def job_init_excellon(obj_fin, app_obj):
                            ...
                            for tool in panel_source_obj.tools:
                                if panel_source_obj.tools[tool]["drills"]:
                                    ...
                                    for drill in panel_source_obj.tools[tool]["drills"]:
                                        ...
                                        # offset / panelization
                                        point_offseted = affinity.translate(
                                        ...
                                        drill_nr += 1
                                        disp_number = int(
                                            np.interp(drill_nr, [0, geo_len_drills], [0, 100])
                                        )
                                        if old_disp_number < disp_number <= 100:
                                            self.app.proc_container.update_view_text(
                                                " %s: %d D:%d%%"
                                                % (_("Copy"), int(element), disp_number)
                                            )
                                ...
                                if panel_source_obj.tools[tool]["slots"]:
                                    ...
                                    for slot in panel_source_obj.tools[tool]["slots"]:
                                        ...
                                        # offset / panelization
                                        start_offseted = affinity.translate(
                                            slot[0], currentx, currenty
                                        )
                                        stop_offseted = affinity.translate(
                                            slot[1], currentx, currenty
                                        )
                                        offseted_slot = (start_offseted, stop_offseted)
                                        obj_fin.tools[tool]["slots"].append(offseted_slot)

                                        # update progress
                                        slot_nr += 1
                                        disp_number = int(
                                            np.interp(slot_nr, [0, geo_len_slots], [0, 100])
                                        )
                                        if old_disp_number < disp_number <= 100:
                                            self.app.proc_container.update_view_text(
                                                " %s: %d S:%d%%"
                    ...
                    obj_fin.source_file = self.app.f_handlers.export_excellon(
                        obj_name=self.outname, filename=None, local_use=obj_fin, use_thread=False
                    )
                ...
                def job_init_geometry(obj_fin, app_obj):
                            ...
                            if panel_source_obj.kind == "geometry":
                                if panel_source_obj.multigeo is True:
                                    for tool in panel_source_obj.tools:
                                        ...
                                        try:
                                            geo_len = len(
                                                panel_source_obj.tools[tool]["solid_geometry"]
                                            )
                                        except TypeError:
                                            geo_len = 1
                                        ...
                                        # panelization
                                        pol_nr = 0
                                        for geo_el in panel_source_obj.tools[tool][
                                            "solid_geometry"
                                        ]:
                                            ...
                                            pol_nr += 1
                                            disp_number = int(
                                                np.interp(pol_nr, [0, geo_len], [0, 100])
                                            )
                                            if old_disp_number < disp_number <= 100:
                                                app_obj.proc_container.update_view_text(
                                                    " %s: %d %d%%"
                                    ...
                                    try:
                                        ...
                                        trans_geo = translate_recursion(
                                            panel_source_obj.solid_geometry
                                        )
                                        obj_fin.solid_geometry.append(trans_geo)
                            # Will panelize a Gerber Object
                                ...
                                for apid in panel_source_obj.apertures:
                                    ...
                                    if "geometry" in panel_source_obj.apertures[apid]:
                                        ...
                                        try:
                                            geo_len = len(
                                                panel_source_obj.apertures[apid]["geometry"]
                                            )
                                        except TypeError:
                                            geo_len = 1
                                        ...
                                        for el in panel_source_obj.apertures[apid]["geometry"]:
                                            ...
                                            obj_fin.apertures[apid]["geometry"].append(
                                                deepcopy(new_el)
                                            )
                    ...
                    if panel_type == "gerber":
                        obj_fin.source_file = self.app.f_handlers.export_gerber(
                            obj_name=self.outname,
                            filename=None,
                            local_use=obj_fin,
                            use_thread=False,
                        )
                    if panel_type == "geometry":
                        obj_fin.source_file = self.app.f_handlers.export_dxf(
                            obj_name=self.outname,
                            filename=None,
                            local_use=obj_fin,
                            use_thread=False,
                ...
                self.app.inform.emit(
                    "%s: %d" % (_("Generating panel... Spawning copies"), (int(rows * columns)))
                )
        ...
        if self.constrain_flag is False:
            ...
            self.app.inform.emit(
                _(
                    "{text} Too big for the constrain area. "
                    "Final panel has {col} columns and {row} rows"
                ).format(text="[WARNING] ", col=columns, row=rows)
            )
...
class PanelizeUI:
    ...
    def __init__(self, layout, app):
        ...
        self.type_obj_combo.setItemIcon(
            0, QtGui.QIcon(self.app.resource_location + "/flatcam_icon16.png")
        )
        self.type_obj_combo.setItemIcon(1, QtGui.QIcon(self.app.resource_location + "/drill16.png"))
        self.type_obj_combo.setItemIcon(
            2, QtGui.QIcon(self.app.resource_location + "/geometry16.png")
        )
        ...
        self.type_box_combo.setItemIcon(
            0, QtGui.QIcon(self.app.resource_location + "/flatcam_icon16.png")
        )
        self.type_box_combo.setItemIcon(
            1, QtGui.QIcon(self.app.resource_location + "/geometry16.png")
        )
        ...
        self.columns_label.setToolTip(_("Number of columns of the desired panel"))
        ...
        self.rows_label.setToolTip(_("Number of rows of the desired panel"))
        ...
        self.panel_type_radio = RadioSet(
            [{"label": _("Gerber"), "value": "gerber"}, {"label": _("Geo"), "value": "geometry"}]
        )
        ...
        self.constrain_sel = OptionalInputSection(
            self.constrain_cb,
            [self.x_width_lbl, self.x_width_entry, self.y_height_lbl, self.y_height_entry],
        )
        ...
        self.panelize_object_button.setIcon(
            QtGui.QIcon(self.app.resource_location + "/panelize16.png")
        )
