...
class Distance(AppTool):
    ...
    def activate_measure_tool(self):
        ...
        if self.app.call_source == "app":
            ...
            if self.app.is_legacy is False:
                ...
                self.canvas.graph_event_disconnect(
                    "mouse_press", self.app.geo_editor.on_canvas_click
                )
                self.canvas.graph_event_disconnect(
                    "mouse_release", self.app.geo_editor.on_geo_click_release
                )
            else:
            ...
            if self.app.is_legacy is False:
                ...
                self.canvas.graph_event_disconnect(
                    "mouse_release", self.app.exc_editor.on_exc_click_release
                )
            else:
            ...
            if self.app.is_legacy is False:
                ...
                self.canvas.graph_event_disconnect(
                    "mouse_release", self.app.grb_editor.on_grb_click_release
                )
            else:
    ...
    def deactivate_measure_tool(self):
        ...
        if self.original_call_source == "app":
            ...
            self.app.geo_editor.mm = self.canvas.graph_event_connect(
                "mouse_move", self.app.geo_editor.on_canvas_move
            )
            self.app.geo_editor.mp = self.canvas.graph_event_connect(
                "mouse_press", self.app.geo_editor.on_canvas_click
            )
            self.app.geo_editor.mr = self.canvas.graph_event_connect(
                "mouse_release", self.app.geo_editor.on_geo_click_release
            )

        elif self.original_call_source == "exc_editor":
            self.app.exc_editor.mm = self.canvas.graph_event_connect(
                "mouse_move", self.app.exc_editor.on_canvas_move
            )
            self.app.exc_editor.mp = self.canvas.graph_event_connect(
                "mouse_press", self.app.exc_editor.on_canvas_click
            )
            self.app.exc_editor.mr = self.canvas.graph_event_connect(
                "mouse_release", self.app.exc_editor.on_exc_click_release
            )

        elif self.original_call_source == "grb_editor":
            self.app.grb_editor.mm = self.canvas.graph_event_connect(
                "mouse_move", self.app.grb_editor.on_canvas_move
            )
            self.app.grb_editor.mp = self.canvas.graph_event_connect(
                "mouse_press", self.app.grb_editor.on_canvas_click
            )
            self.app.grb_editor.mr = self.canvas.graph_event_connect(
                "mouse_release", self.app.grb_editor.on_grb_click_release
            )
        ...
        if event.button == 1:
            ...
            if self.ui.snap_center_cb.get_value() is False:
                ...
                if self.original_call_source == "exc_editor":
                    for storage in self.app.exc_editor.storage_dict:
                        __, st_closest_shape = self.app.exc_editor.storage_dict[storage].nearest(
                            pos
                        )
                    ...
                    for storage in self.app.grb_editor.storage_dict:
                        try:
                            for shape_stored in self.app.grb_editor.storage_dict[storage][
                                "geometry"
                            ]:
            ...
            if self.rel_point1 is None:
                self.app.ui.rel_position_label.setText(
                    "<b>Dx</b>: %.*f&nbsp;&nbsp;  <b>Dy</b>: "
                    "%.*f&nbsp;&nbsp;&nbsp;&nbsp;" % (self.decimals, 0.0, self.decimals, 0.0)
                )
                self.rel_point1 = pos
            else:
    ...
    def calculate_distance(self, pos):
        if len(self.points) == 1:
            self.ui.start_entry.set_value(
                "(%.*f, %.*f)" % (self.decimals, pos[0], self.decimals, pos[1])
            )
            ...
            self.ui.stop_entry.set_value(
                "(%.*f, %.*f)" % (self.decimals, pos[0], self.decimals, pos[1])
            )

            self.app.inform.emit(
                "{tx1}: {tx2} D(x) = {d_x} | D(y) = {d_y} | {tx3} = {d_z}".format(
                    tx1=_("MEASURING"),
                    tx2=_("Result"),
                    tx3=_("Distance"),
        ...
        try:  # May fail in case mouse not within axes
            ...
            if len(self.points) == 1:
                ...
                try:
                    ...
                    log.debug(
                        "Distance.on_mouse_move_meas() -> update utility geometry -> %s" % str(e)
                    )
                    pass
    ...
    def utility_geometry(self, pos):
        ...
        self.sel_shapes.add(
            meas_line, color=color, update=True, layer=0, tolerance=None, linewidth=2
        )
...
class DistUI:
    ...
    def __init__(self, layout, app):
        ...
        self.units_value = FCLabel(
            "%s" % str({"mm": _("METRIC (mm)"), "in": _("INCH (in)")}[self.units])
        )
