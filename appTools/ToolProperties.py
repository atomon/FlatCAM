...
class Properties(AppTool):
    ...
    def addItems(self, obj):
        ...
        obj_type = self.treeWidget.addParent(
            parent, _("TYPE"), expanded=True, color=p_color, font=font
        )
        obj_name = self.treeWidget.addParent(
            parent, _("NAME"), expanded=True, color=p_color, font=font
        )
        dims = self.treeWidget.addParent(
            parent, _("Dimensions"), expanded=True, color=p_color, font=font
        )
        units = self.treeWidget.addParent(
            parent, _("Units"), expanded=True, color=p_color, font=font
        )
        ...
        self.treeWidget.addChild(
            obj_type,
            ["%s:" % _("Object Type"), ("%s" % (obj.kind.upper()))],
            True,
            font=font,
            font_items=1,
        )
        ...
        o_unit = {"in": _("Inch"), "mm": _("Metric"), "inch": _("Inch"), "metric": _("Metric")}[
