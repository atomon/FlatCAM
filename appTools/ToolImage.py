    ...
    def on_file_importimage(self):
        ...
        try:
            filename, _f = QtWidgets.QFileDialog.getOpenFileName(
                caption=_("Import IMAGE"), directory=self.app.get_last_folder(), filter=_filter
            )
        except TypeError:
            filename, _f = QtWidgets.QFileDialog.getOpenFileName(
                caption=_("Import IMAGE"), filter=filter
            )
        ...
        if filename == "":
            ...
            self.app.worker_task.emit(
                {"fcn": self.import_image, "params": [filename, type_obj, dpi, mode, mask]}
            )

    def import_image(
        self, filename, o_type=_("Gerber"), dpi=96, mode="black", mask=None, outname=None
    ):
        """
        ...
        if o_type is None or o_type == _("Geometry"):
            ...
            self.app.inform.emit(
                "[ERROR_NOTCL] %s"
                % _(
                    "Not supported type is picked as parameter. "
...
class ImageUI:
    ...
    def __init__(self, layout, app):
        ...
        self.tf_type_obj_combo.setItemIcon(
        ...
        self.detail_label = QtWidgets.QLabel(
            "<font size=4><b>%s:</b></font>" % _("Level of detail")
        )
        ...
        self.image_type = RadioSet(
            [{"label": "B/W", "value": "black"}, {"label": "Color", "value": "color"}]
        )
