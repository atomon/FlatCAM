...
class bilinearInterpolator:
    ...
    def Interpolate(self, point):
        ...
        def interpolatePoint(p1, p2, pt, axis):
            return (p2[2] * (pt[axis] - p1[axis]) + p1[2] * (p2[axis] - pt[axis])) / (
                p2[axis] - p1[axis]
            )
        ...
        if point[1] < self.yMin:
            ...
            return interpolatePoint(
                self.probedGrid[ix1][self.yCount - 1],
                self.probedGrid[ix2][self.yCount - 1],
                point,
                0,
            )
        else:
            ...
            if lin:
                return interpolatePoint(
                    self.probedGrid[ix1][iy1], self.probedGrid[ix1][iy2], point, 1
                )
