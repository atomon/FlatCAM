...
class TclShellTest(unittest.TestCase):
    ...
    def test_gerber_flow(self):
        ...
        # -----------------------------------------
        cmd = gbr_cmd.format(
            path=self.gerber_files, filename=self.copper_top_filename, outname=self.gerber_top_name
        )
        self.fc.exec_command_test(cmd)
        gerber_top_obj = self.fc.collection.get_by_name(self.gerber_top_name)
        self.assertTrue(
            isinstance(gerber_top_obj, FlatCAMGerber),
        ...
        self.assertTrue(
            isinstance(gerber_bottom_obj, FlatCAMGerber),
            "Expected FlatCAMGerber, instead, %s is %s"
        ...
        # --------------------------------------------
        cmd = gbr_cmd.format(
            path=self.gerber_files, filename=self.cutout_filename, outname=self.gerber_cutout_name
        )
        self.fc.exec_command_test(cmd)
        gerber_cutout_obj = self.fc.collection.get_by_name(self.gerber_cutout_name)
        self.assertTrue(
            isinstance(gerber_cutout_obj, FlatCAMGerber),
            "Expected FlatCAMGerber, instead, %s is %s"
        ...
        cmd = "isolate {objname} -dia {dia}".format(
            objname=self.gerber_cutout_name, dia=self.engraver_diameter
        )
        self.fc.exec_command_test(cmd)
        ...
        cmd = "delete {objname}".format(objname=self.gerber_cutout_name + "_iso")
        self.fc.exec_command_test(cmd)
        ...
        self.assertTrue(
            isinstance(obj, FlatCAMGeometry),
        ...
        self.fc.exec_command_test(
            "mirror %s -box %s -axis X" % (self.gerber_bottom_name, self.gerber_cutout_name)
        )
        self.fc.exec_command_test(
            "mirror %s -box %s -axis X" % (self.gerber_cutout_name, self.gerber_cutout_name)
        )
        ...
        self.fc.exec_command_test(
            ...
            % (
                self.gerber_cutout_name,
                self.engraver_diameter,
                self.gerber_cutout_name + "_bottom_iso",
            )
        )
        self.fc.exec_command_test(
        ...
        self.assertEqual(len(names), 5, "Expected 5 objects, found %d" % len(names))

        # isolate traces
        self.fc.exec_command_test(
            "isolate %s -dia %f" % (self.gerber_top_name, self.engraver_diameter)
        )
        self.fc.exec_command_test(
            "isolate %s -dia %f" % (self.gerber_bottom_name, self.engraver_diameter)
        )
        ...
        self.fc.exec_command_test(
            ...
            % (
                self.gerber_top_name + "_join_iso",
                self.gerber_top_name + "_iso",
        ...
        self.fc.exec_command_test(
            "join_geometries %s %s %s"
            % (
                self.gerber_bottom_name + "_join_iso",
                self.gerber_bottom_name + "_iso",
                self.gerber_cutout_name + "_bottom_iso_exterior",
            )
        )
        ...
        names = self.fc.collection.get_names()
        self.assertEqual(len(names), 9, "Expected 9 objects, found %d" % len(names))
    ...
    def test_excellon_flow(self):
        ...
        self.fc.exec_command_test(
            "mirror %s -box %s -axis X" % (self.excellon_name, self.gerber_cutout_name)
        )
