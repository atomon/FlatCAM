...
class PaintConnectTest(PaintTestCase):
    ...
    def test_no_jump1(self):
        ...
        paths = [LineString([[0, 2], [2, 5]]), LineString([[2, 0], [5, 2]])]
        for p in paths:
            print(p)
...
class PaintConnectTest2(PaintTestCase):
    ...
    def setUp(self):
        self.boundary = Polygon([[0, 0], [0, 5], [5, 5], [5, 0]])
        self.boundary = self.boundary.difference(Polygon([[2, 1], [3, 1], [3, 4], [2, 4]]))

    def test_no_jump3(self):
        print("TEST: No jump expected")
        paths = [LineString([[0.5, 1], [1.5, 3]]), LineString([[4, 1], [4, 4]])]
        for p in paths:
            print(p)
