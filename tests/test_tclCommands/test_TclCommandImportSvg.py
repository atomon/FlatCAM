...
def test_import_svg(self):
    ...
    for svg_file in file_list:
        ...
        self.assertTrue(
            ...
            "Expected FlatCAMGeometry, instead, %s is %s" % (svg_file, type(obj)),
        )

        # import  with outname
        outname = "%s-%s" % (self.geometry_name, svg_file)
        self.fc.exec_command_test(
            'import_svg "%s/%s" -outname "%s"' % (self.svg_files, svg_file, outname)
        )

        obj = self.fc.collection.get_by_name(outname)
        self.assertTrue(
            isinstance(obj, FlatCAMGeometry),
            "Expected FlatCAMGeometry, instead, %s is %s" % (outname, type(obj)),
        )
...
def test_import_svg_as_geometry(self):
    ...
    self.fc.exec_command_test(
        'import_svg "%s/%s" -type geometry -outname "%s"'
        % (self.svg_files, self.svg_filename, self.geometry_name)
    )

    obj = self.fc.collection.get_by_name(self.geometry_name)
    self.assertTrue(
        isinstance(obj, FlatCAMGeometry) and not isinstance(obj, FlatCAMGerber),
...
def test_import_svg_as_gerber(self):
    ...
    self.fc.exec_command_test(
        'import_svg "%s/%s" -type gerber -outname "%s"'
