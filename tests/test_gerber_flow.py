...
class GerberFlowTestCase(unittest.TestCase):
    ...
    def test_flow(self):
        ...
        self.assertTrue(
            ...
            "Expected FlatCAMGerber, instead, %s is %s" % (gerber_name, type(gerber_obj)),
        )
        ...
        self.assertTrue(geo_name in names, "Object named %s not found." % geo_name)
        ...
        # ---------------------------------------------
        names = self.fc.collection.get_names()
        self.assertEqual(len(names), 3, "Expected 3 objects, found %d" % len(names))
