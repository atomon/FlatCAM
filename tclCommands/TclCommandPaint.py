...
class TclCommandPaint(TclCommand):
    ...
    description = "%s %s" % (
        "--",
        "Paint polygons in the specified object by covering them with toolpaths.",
    )
    ...
    help = {
        ...
        "args": collections.OrderedDict(
            [
                ...
                (
                    "tooldia",
                    "Diameter of the tools to be used. Can be a comma separated list of diameters.\n"
                ...
                (
                    "order",
                    'Can have the values: "no", "fwd" and "rev". String.\n'
                ...
                (
                    "method",
                    'Algorithm for painting. Can be: "standard", "seed", "lines", "laser_lines", "combo".',
                ),
        ...
        "examples": [
            "paint obj_name -tooldia 0.3 -offset 0.1 -method 'seed' -all",
    ...
    def execute(self, args, unnamed_args):
        ...
        for tool in tools:
            ...
            paint_tools[int(tooluid)]["data"]["tooldia"] = self.app.dec_format(
                float(tool), self.app.decimals
            )
        ...
        if "all" in args:
            self.app.paint_tool.paint_poly_all(
                obj=obj,
                tooldia=tooldia,
                order=order,
                method=method,
        ...
        if "single" in args:
            if not args["single"] or args["single"] == "":
                ...
                ret_val = self.app.paint_tool.paint_poly(
                    obj=obj,
                    inside_pt=[x, y],
                    tooldia=tooldia,
                    order=order,
        ...
        if "box" in args:
            ...
            self.app.paint_tool.paint_poly_ref(
                obj=obj,
                sel_obj=box_obj,
                tooldia=tooldia,
                order=order,
        ...
        self.raise_tcl_error(
            "%s:"
            % _("None of the following args: 'box', 'single', 'all' were used.\n" "Paint failed.")
        )
