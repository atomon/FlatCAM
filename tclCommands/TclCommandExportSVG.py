...
class TclCommandExportSVG(TclCommand):
    ...
    arg_names = collections.OrderedDict(
        [("name", str), ("filename", str), ("scale_stroke_factor", float)]
    )

    # Dictionary of types from Tcl command, needs to be ordered , this  is  for options  like -optionname value
    option_types = collections.OrderedDict([("scale_stroke_factor", float)])
    ...
    help = {
        ...
        "args": collections.OrderedDict(
            [
                ...
                (
                    "filename",
                    "Absolute path to file to export.\n"
                    "WARNING: no spaces are allowed. If unsure enclose the entire path with quotes.",
                ),
                (
                    "scale_stroke_factor",
                    "Multiplication factor used for scaling line widths during export.",
                ),
            ]
        ),
