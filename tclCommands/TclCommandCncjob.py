...
class TclCommandCncjob(TclCommandSignaled):
    ...
    help = {
        ...
        "args": collections.OrderedDict(
            [
                ...
                (
                    "extracut_length",
                    "The value for extra cnccut over the first point in path,in the job end; float",
                ),
                (
                    "dpp",
                    "If present then use multidepth cnc cut. Height of one layer for multidepth.",
                ),
                (
                    "toolchangez",
    ...
    def execute(self, args, unnamed_args):
        ...
        args["dia"] = (
            args["dia"]
            if "dia" in args and args["dia"]
            else self.app.defaults["geometry_cnctooldia"]
        )

        args["z_cut"] = (
            args["z_cut"]
            if "z_cut" in args and args["z_cut"]
            else self.app.defaults["geometry_cutz"]
        )
        args["z_move"] = (
            args["z_move"]
            if "z_move" in args and args["z_move"]
            else self.app.defaults["geometry_travelz"]
        )

        args["feedrate"] = (
            args["feedrate"]
            if "feedrate" in args and args["feedrate"]
            else self.app.defaults["geometry_feedrate"]
        )
        args["feedrate_z"] = (
            args["feedrate_z"]
        ...
        args["startz"] = (
            args["startz"]
            if "startz" in args and args["startz"]
            else self.app.defaults["geometry_startz"]
        )
        args["endz"] = (
            args["endz"] if "endz" in args and args["endz"] else self.app.defaults["geometry_endz"]
        )
        ...
        if len(eval(args["endxy"])) != 2:
            self.raise_tcl_error(
                "The entered value for 'endxy' needs to have the format x,y or "
                "in format (x, y) - no spaces allowed. But always two comma separated values."
            )

        args["spindlespeed"] = (
            args["spindlespeed"] if "spindlespeed" in args and args["spindlespeed"] != 0 else None
        )
        ...
        args["pp"] = (
            args["pp"] if "pp" in args and args["pp"] else self.app.defaults["geometry_ppname_g"]
        )
        ...
        for arg in args:
            if arg == "toolchange_xy" or arg == "spindlespeed" or arg == "startz":
                ...
                if args[arg] is None:
                    ...
                    if muted is False:
                        self.raise_tcl_error(
                            "One of the command parameters that have to be not None, is None.\n"
            ...
            for tool_uid in list(local_tools_dict.keys()):
                if "data" in local_tools_dict[tool_uid]:
                    ...
                    if args["extracut"] is True:
                        local_tools_dict[tool_uid]["data"]["extracut_length"] = args[
                            "extracut_length"
                        ]
                    else:
