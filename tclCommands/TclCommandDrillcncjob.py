    ...
    help = {
        ...
        "args": collections.OrderedDict(
            [
                ...
                (
                    "dpp",
                    "Progressive drilling into material with a specified step (example: 0.7). Positive value.",
                ),
                ...
                (
                    "toolchangexy",
                    "The X,Y coordinates at Toolchange event in format (x, y) (example: (30.0, 15.2) or "
                ...
                (
                    "endxy",
                    "The X,Y coordinates at job end in format (x, y) (example: (2.0, 1.2) or without parenthesis"
                    "like: 0.3,1.0). WARNING: no spaces allowed in the value.",
                ),
                (
                    "dwelltime",
                    "Time to pause to allow the spindle to reach the full speed.\n"
                ...
                (
                    "opt_type",
                    "Name of move optimization type. B by default for Basic OR-Tools, M for Metaheuristic OR-Tools"
                ...
                (
                    "diatol",
                    "Tolerance. Percentange (0.0 ... 100.0) within which dias in drilled_dias will be judged to be "
                ...
                (
                    "muted",
                    "It will not put errors in the Shell or status bar. Can be True (1) or False (0).",
                ),
                ("outname", "Name of the resulting Geometry object."),
            ]
        ),
        "examples": [
            "drillcncjob test.TXT -drillz -1.5 -travelz 14 -feedrate_z 222 -feedrate_rapid 456 "
    ...
    def execute(self, args, unnamed_args):
        ...
        def job_init(job_obj, app_obj):
                ...
                if "drilled_dias" in args and args["drilled_dias"] != "all":
                    ...
                    for tool in obj.tools:
                        for req_dia in diameters:
                            obj_dia_form = float(
                                "%.*f" % (obj.decimals, float(obj.tools[tool]["tooldia"]))
                            )
                    ...
                    if nr_diameters > 0:
                        if muted is False:
                            self.raise_tcl_error(
                                "One or more tool diameters of the drills to be drilled passed to the "
            ...
            if tools == "all":
                ...
                for tool_no in use_tools:
                    ...
                    used_tools_info.append(
                        [str(tool_no), str(tool_dia_used), str(drill_cnt), str(slot_cnt)]
                    )

            drillz = (
                args["drillz"]
                if "drillz" in args and args["drillz"] is not None
                else obj.options["tools_drill_cutz"]
            )
            ...
            if len(eval(xy_toolchange)) != 2:
                self.raise_tcl_error(
                    "The entered value for 'toolchangexy' needs to have the format x,y or "
                    "in format (x, y) - no spaces allowed. But always two comma separated values."
                )

            endz = (
                args["endz"]
                if "endz" in args and args["endz"] is not None
                else self.app.defaults["tools_drill_endz"]
            )
            ...
            if len(eval(xy_end)) != 2:
                self.raise_tcl_error(
                    "The entered value for 'xy_end' needs to have the format x,y or "
                    "in format (x, y) - no spaces allowed. But always two comma separated values."
                )
            ...
            pp_excellon_name = (
                args["pp"]
                if "pp" in args and args["pp"]
                else self.app.defaults["tools_drill_ppname_e"]
            )
            ...
            job_obj.z_move = (
                float(args["travelz"])
                if "travelz" in args and args["travelz"]
                else self.app.defaults["tools_drill_travelz"]
            )

            job_obj.feedrate = (
                float(args["feedrate_z"])
                if "feedrate_z" in args and args["feedrate_z"]
                else self.app.defaults["tools_drill_feedrate_z"]
            )
            job_obj.z_feedrate = (
                float(args["feedrate_z"])
                if "feedrate_z" in args and args["feedrate_z"]
            ...
            job_obj.feedrate_rapid = (
                float(args["feedrate_rapid"])
                if "feedrate_rapid" in args and args["feedrate_rapid"]
                else self.app.defaults["tools_drill_feedrate_rapid"]
            )
            ...
            for t_item in job_obj.exc_cnc_tools:
                job_obj.exc_cnc_tools[t_item]["data"]["tools_drill_offset"] = float(
                    job_obj.exc_cnc_tools[t_item]["offset_z"]
                ) + float(drillz)
                job_obj.exc_cnc_tools[t_item]["data"]["tools_drill_ppname_e"] = job_obj.options[
                    "ppname_e"
                ]
