        ...
        if "name" in args:
            ...
            if not args:
                ...
                try:
                    ...
                    self.app.log.debug(
                        "tclCommands.TclCommandHelp() when run as 'help' --> %s" % str(err)
                    )
                    displayed_text = [
                        "> %s" % cmd for cmd in sorted(self.app.shell.tcl_commands_storage)
                    ]

                cmd_enum += "<br>".join(displayed_text)
                cmd_enum += "<br><br>%s<br>%s<br><br>" % (
                    _("Type help <command_name> for usage."),
                    _("Example: help open_gerber"),
                )
