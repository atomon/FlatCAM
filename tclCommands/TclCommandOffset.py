...
class TclCommandOffset(TclCommand):
    ...
    description = "%s %s" % (
        "--",
        "Will offset the geometry of a named object. Does not create a new object.",
    )

    # Dictionary of types from Tcl command, needs to be ordered
    arg_names = collections.OrderedDict([("name", str), ("x", float), ("y", float)])
    ...
    help = {
        ...
        "args": collections.OrderedDict(
            [
                ...
                (
                    "x",
                    "Offset distance in the X axis. If it is not used it will be assumed to be 0.0",
                ),
                (
                    "y",
                    "Offset distance in the Y axis. If it is not used it will be assumed to be 0.0",
                ),
            ]
        ),
