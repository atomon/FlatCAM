...
class TclCommandBbox(TclCommand):
    ...
    description = "%s %s" % (
        "--",
        "Creates a rectangular Geometry object that surrounds the object.",
    )
    ...
    option_types = collections.OrderedDict([("outname", str), ("margin", float), ("rounded", str)])
    ...
    help = {
        ...
        "args": collections.OrderedDict(
            [
                ...
                (
                    "margin",
                    "Distance of the edges of the box to the nearest polygon." "Float number.",
                ),
                (
                    "rounded",
                    "If the bounding box has to have rounded corners their radius is equal to the margin. "
                    "True (1) or False (0).",
                ),
    ...
    def execute(self, args, unnamed_args):
        ...
        if obj.kind != "gerber" and obj.kind != "geometry":
            self.raise_tcl_error(
                "%s %s: %s." % (_("Expected GerberObject or GeometryObject, got"), name, type(obj))
            )
