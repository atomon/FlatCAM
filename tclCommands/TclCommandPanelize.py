...
class TclCommandPanelize(TclCommand):
    ...
    description = "%s %s" % (
        "--",
        "Create a new object with an array of duplicates of the original geometry, "
        "arranged in a grid.",
    )
    ...
    help = {
        ...
        "args": collections.OrderedDict(
            [
                ...
                (
                    "box",
                    "Name of object which acts as box (cutout for example.)"
                ...
                (
                    "use_thread",
                    "False (0) = non-threaded execution or True (1) = threaded execution",
                ),
            ]
        ),
        "examples": [
            ...
            if obj is not None:
                    ...
                    for option in obj.options:
                        if option != "name":
                            try:
                                ...
                                app_obj.log.debug(
                                    "TclCommandPanelize.execute().panelize2() --> %s" % str(e)
                                )
                            ...
                            if "slots" in obj.tools:
                                for slot_tuple in obj.tools["slots"]:
                                    start_offseted = affinity.translate(
                                        slot_tuple[0], currentx, currenty
                                    )
                                    stop_offseted = affinity.translate(
                                        slot_tuple[1], currentx, currenty
                                    )
                                    obj_fin.tools["slots"].append((start_offseted, stop_offseted))
                            currentx += lenghtx
                        currenty += lenghty
                    ...
                    for row in range(rows):
                        ...
                        for col in range(columns):
                            if obj.kind == "geometry":
                                if obj.multigeo is True:
                                    for tool in obj.tools:
                                        obj_fin.tools[tool]["solid_geometry"].append(
                                            translate_recursion(obj.tools[tool]["solid_geometry"])
                                        )
                                else:
                ...
                if obj.kind == "excellon":
                    self.app.app_obj.new_object(
                        "excellon", outname, job_init_excellon, plot=False, autoselected=True
                    )
                else:
                    self.app.app_obj.new_object(
                        "geometry", outname, job_init_geometry, plot=False, autoselected=True
                    )
