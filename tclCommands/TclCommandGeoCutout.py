...
class TclCommandGeoCutout(TclCommandSignaled):
    ...
    description = "%s %s" % (
        "--",
        "Creates board cutout from an object (Gerber or Geometry) of any shape.",
    )
    ...
    option_types = collections.OrderedDict(
        [("dia", float), ("margin", float), ("gapsize", float), ("gaps", str), ("outname", str)]
    )
    ...
    help = {
        ...
        "args": collections.OrderedDict(
            [
                ...
                (
                    "gaps",
                    "type of gaps. Can be: 'tb' = top-bottom, 'lr' = left-right, '2tb' = 2top-2bottom, "
    ...
    def execute(self, args, unnamed_args):
        ...
        if "name" in args:
            ...
            self.app.inform.emit(
                "[WARNING] %s"
                % _("The name of the object for which cutout is done is missing. Add it and retry.")
            )
            return
        ...
        if gaps not in ["lr", "tb", "2lr", "2tb", "4", "8"]:
            self.app.inform.emit(
                "[WARNING] %s"
                % _("Gaps value can be only one of: 'lr', 'tb', '2lr', '2tb', 4 or 8.")
            )
            return
        ...
        if cutout_obj.kind == "geometry":
            ...
            def geo_init(geo_obj, app_obj):
                ...
                if gaps_u == 4 or gaps_u == "lr":
                    geo = substract_rectangle_geo(
                        geo, xmin - gapsize, py - gapsize, xmax + gapsize, py + gapsize
                    )

                if gaps_u == 4 or gaps_u == "tb":
                    geo = substract_rectangle_geo(
                        geo, px - gapsize, ymin - gapsize, px + gapsize, ymax + gapsize
                    )
            ...
            def geo_init(geo_obj, app_obj):
                try:
                    geo = cutout_obj.isolation_geometry(
                        (dia / 2), iso_type=0, corner=2, follow=None
                    )
