...
class TclCommandBounds(TclCommand):
    ...
    description = "%s %s" % (
        "--",
        "Return in the console a list of bounds values for a list of objects.",
    )

    # Dictionary of types from Tcl command, needs to be ordered
    arg_names = collections.OrderedDict([("objects", str)])
    ...
    def execute(self, args, unnamed_args):
        ...
        if "objects" in args:
            try:
                obj_list = [
                    str(obj_name) for obj_name in str(args["objects"]).split(",") if obj_name != ""
                ]
            ...
            if not obj_list:
                self.raise_tcl_error(
                    "%s: %s:"
                    % (
                        _("Expected a list of objects names separated by comma. Got"),
                        str(args["objects"]),
                    )
                )
                return "fail"
